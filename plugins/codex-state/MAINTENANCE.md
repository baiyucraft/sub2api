...
本插件在独立目录构建；宿主只保留通用插件管理和 Native 声明式渲染能力。不要恢复内置 STATE 管理 API、账号编辑入口或账号用量专用展示，不把普通账号 `extra` / credentials 暴露到插件 UI。新安装所有开关关闭，不从旧内置设置或旧票据自动迁移。当前默认页面是 `ui/admin-ui.json`，旧 `ui/dist` 仅作为 iframe 回滚与兼容资源。

配置的 `enabled` 是布尔总开关，状态中的 `active` / `ready` 是只读对象。配置 schema 版本 `1`、模型名与 Pro / Team 值不能静默改变。变更配置、状态、动作或 host SDK 契约时，先明确兼容策略，再同步 Native 页面定义、双语翻译、旧 iframe 回滚资源、测试和 manifest。
...
最低能力为 `plugin_protocol=1`、`transport_api=1`、`admin_ui=1`、`host_service_api=2`；Native 包不依赖 `ui_bridge` 管理页能力。原七个必需 feature 为 `scoped-routing.v1`、`admission.v1`、`resources.v1`、`actions.v1`、`state-cas.v1`、`leases.v1`、`oauth-like.v1`；当前打包器另外要求 `request-completion.v1` 和 `config-secrets.v1`，共九项。宿主版本字符串不能取代能力协商，旧 iframe 插件缺省可选能力字段的行为也须回归。

- `GET /api/v1/admin/plugins/:id/resources` 在停止态可读；严格投影目录元数据，不携带账号 extra、凭据、代理用户或密码。
- `POST /api/v1/admin/plugins/:id/actions` 请求 `{action_id,name,payload}`，响应 `{action_id,accepted,status,result}`。`harvest` / `cancel` 使用 `{account_id,model}`；Native 页面还使用 `accounts.bulk_update` 的 `{account_ids,models}`，只覆盖明确字段并通过 `result.config` 回填宿主草稿。页面必须为启用/停用和 Pro/Team 提供明确按钮，不能用默认值不明的 checkbox 模拟“未修改”；停止态禁止动作，step-up 后仍需重查运行态。
...
- `ui/admin-ui.json` 必须提供 `title_key` / `description_key` / `label_key` 对应的 `en`、`zh` 翻译；页面结构至少覆盖概览、账号数据表、筛选与选择、批量明确操作和日志表格。
- 账号资源只能绑定固定白名单字段；禁止使用 `/item` 这类整对象 `key_value` 展示，以免未来资源字段把 STATE、凭据、代理 URL 或内部指纹带入页面。
- 日志表格只绑定时间、账号、模型和脱敏诊断码，必要的时间字段声明 `format=date`；页面文案明确不展示原始错误、票据、令牌或代理凭据。
- Bridge 必须检查 iframe source、会话 token、request_id、消息类型。关闭 / 切换 UI 后旧响应不能写入新会话，不能在旧 step-up 返回后执行 mutation。
- Native 页面不得引入脚本、HTML、远程资源、自由表达式或宿主 DOM 访问；其定义必须位于 `ui/` 并进入 manifest 哈希。iframe 回滚路径不得增加 `allow-same-origin`、管理员 token 或网络权限；不使用 localStorage 保存代理 URL 或配置。两条路径的状态、日志、错误文本中不得出现完整 URL、authorization、Cookie、STATE envelope 或指纹。
...
在 `plugins/codex-state/ui/` 中执行旧 iframe 回滚资源的构建与测试：
...
还需在本地测试宿主中优先检查 Native 页面：`/admin/plugins/:pluginKey` 深链接、插件管理入口高亮、中文 / 英文翻译、概览、账号筛选、跨页选择、批量明确按钮、`result.config` 回填、日志表格、停用态、未保存草稿和敏感字段白名单。随后在真实 `sandbox="allow-scripts"` 且 `connect-src 'none'` 的环境回归旧 iframe：布局、初始关闭、筛选、草稿、保存、取消及敏感错误投影。凭据回归必须断言 config.load / config.save / plugin.secrets.edit 的全部 iframe 响应、DOM 和草稿都没有秘密原文，同时验证普通保存留密、明确清空、无值回显、旧插件兼容及首次保存。测试使用合成目录 / 状态，不连接生产、真实账号或上游采集服务。Go 测试由运行时负责人单独安排；上述前端命令不代替 Go 回归。

## 打包与升级

1. 先完成旧 iframe UI 构建，检查 `ui/dist/index.html` 和引用的 JS / CSS 均存在；同时检查 `ui/admin-ui.json` 存在且是声明式 JSON，包含双语键、明确批量 Action 和脱敏日志列，不含 symlink、源码映射秘密或配置草稿。
2. 确认插件根目录的 `LICENSE`、`THIRD_PARTY_NOTICES.md`、`sources.lock.json`、`MAINTENANCE.md` 已更新；工具会把它们放入 `.s2plugin`，缺失则失败。
3. 从插件根运行 `go run ./tools/package --out dist`；工具会同时打入旧 iframe 资源和 `ui/admin-ui.json`，并生成 Manifest v2 `ui.type=native`、`requires.admin_ui=1` 及文件哈希。Native 定义在 amd64/arm64 包中必须语义和字节一致，旧 iframe 资源必须继续存在但只作为回滚依据。正式 fork 包追加 `--signing-key /secure/path/publisher.pkcs8.pem --key-id baiyu-codex-state-v1`。密钥为 Ed25519 PKCS8 PEM，必须位于源码和输出目录之外，且派生公钥必须匹配宿主内置并仅绑定 `baiyu.codex-state` 的信任根。默认构建 Linux amd64 / arm64，可通过 `--arches` 限定。
...
若最终 diff 触及 `backend/pkg/pluginapi/`、宿主 `plugin_*` 服务、管理路由/前端壳、Native renderer、Admin UI 协议/校验、migration 280 后续结构或插件部署配置，则不属于独立包更新，必须同时发布宿主并执行完整 VM Gate。仅修改已支持协议下的 `ui/admin-ui.json` 或插件私有 Action 才可按独立插件包处理。
