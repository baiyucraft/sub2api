...
PROFILE_MIGRATIONS = {
    ...
    "234": {
        migration: f"{index:064x}"
        for index, migration in enumerate(get_profile("234")["migrations"], start=1)
    },
    "235": {
        migration: f"{index:064x}"
        for index, migration in enumerate(get_profile("235")["migrations"], start=1)
    },
    "236": {
        migration: f"{index:064x}"
        for index, migration in enumerate(get_profile("236")["migrations"], start=1)
    },
    "237": {
        migration: f"{index:064x}"
        for index, migration in enumerate(get_profile("237")["migrations"], start=1)
    },
    "238": {
        migration: f"{index:064x}"
        for index, migration in enumerate(get_profile("238")["migrations"], start=1)
    },
    "239": {
        migration: f"{index:064x}"
        for index, migration in enumerate(get_profile("239")["migrations"], start=1)
    },
}
