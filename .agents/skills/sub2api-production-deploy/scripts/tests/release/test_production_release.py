...
class ReleaseClaimScriptTest(unittest.TestCase):
    ...
    def test_vm_gate_signing_rejects_empty_or_partial_payloads(self) -> None:
        validator = (DEPLOY_ROOT / "release" / "vm-validate.sh").read_text(encoding="utf-8")
        self.assertIn("snapshot_digest=$(jq -cS '{current_image_id, production_current_commit_sha, schema_migrations}'", validator)
    ...
    def test_coordinated_restore_reads_redis_password_from_startup_arguments(self) -> None:
        ...
        self.assertIn("redis_backup_persistent", restore)
        self.assertIn("redis_restored_persistent_floor", restore)
        self.assertIn("redis_restored_persistent_floor -ge $redis_backup_persistent", restore)
        self.assertNotIn("redis_backup_dbsize - redis_dbsize)) -eq", restore)
        self.assertNotIn("redis_backup_dbsize - redis_dbsize)) -ge $redis_already_expired", restore)
