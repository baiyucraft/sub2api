...
class ReleaseClaimScriptTest(unittest.TestCase):
    ...
    def test_migration_195_assertion_is_summary_only_and_fail_closed(self) -> None:
        ...
        self.assertIn("if [[ $release_profile == 240 || $release_profile == 241 || $release_profile == 242 || $release_profile == 243 || $release_profile == 244 || $release_profile == 245 || $release_profile == 246 || $release_profile == 247 || $release_profile == 248 || $release_profile == 249 ]]; then", assertion)
    ...
    def test_profile_249_is_health_only_and_does_not_use_canary_credentials(self) -> None:
        validator = (DEPLOY_ROOT / "release" / "vm-validate.sh").read_text(encoding="utf-8")
        ...
        self.assertIn('[[ "$profile" == 249 ]]', validator)
        ...
        self.assertNotIn('    "canary_api_key_id",', profiles[profiles.index('PROFILES["249"]'):])

    def test_profile_249_version_contract_matches_vm_validator(self) -> None:
        validator = (DEPLOY_ROOT / "release" / "vm-validate.sh").read_text(encoding="utf-8")
        profiles = (DEPLOY_ROOT / "release" / "profiles.py").read_text(encoding="utf-8")
        profile_block = profiles[profiles.index('PROFILES["249"]'):]
        self.assertIn('"version": "0.2.4-baiyu"', profile_block)
        self.assertIn('[[ "$version" == 0.2.4-baiyu ]]', validator)
        self.assertIn('[[ $(jq -er \'.parent_profile\' "$manifest") == 248 ]]', validator)
        self.assertIn('[[ $(jq -er \'.new_migrations | length\' "$manifest") == 1 ]]', validator)
        self.assertIn('"269_add_minimax_platform.sql"', validator)
