...
class VMOnlyGateTest(unittest.TestCase):
    ...
    def _bundle(self) -> Path:
        ...
        document = {"gate_version": 2, "profile_id": 253, "manifest": manifest, "evidence": evidence}
    ...
    def test_vm_only_gate_is_verified_by_separate_contract(self) -> None:
        ...
        with mock.patch("release.gate.get_profile", return_value={"gate_schema": 2, "version": "0.2.5-baiyu", "origin": "https://github.com/baiyucraft/sub2api.git"}):
            self.assertEqual(verify_vm_only_gate(bundle, self.public_key, "253")["manifest"]["scope"], "vm-only")

    def test_vm_only_gate_is_not_accepted_as_production_gate(self) -> None:
        bundle = self._bundle()
        with self.assertRaises(Exception):
            verify_gate_v2(bundle, self.public_key, "253")
    ...
    def test_vm_only_manifest_has_no_production_snapshot_fields(self) -> None:
        ...
        with mock.patch("release.manifest.check_output_hidden", return_value="https://github.com/baiyucraft/sub2api.git"):
            manifest = create_vm_only_manifest("a" * 40, profile, "253-aaaaaaaaaaaa-1-aaaaaaaa", "b" * 40, "c" * 64, "d" * 64)
