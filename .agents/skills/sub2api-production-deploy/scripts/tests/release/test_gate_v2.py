...
class GateV2Test(unittest.TestCase):
    ...
    def _document(self, pending: list[dict] | None = None) -> dict:
        ...
        return {"gate_version": 2, "profile_id": 248, "manifest": manifest, "evidence": evidence}
    ...
    def _verify(self, *, allow_historical_runner: bool = False, profile: str = "248") -> dict:
        with (
    ...
    def test_historical_runner_is_allowed_only_for_recovery(self) -> None:
        ...
        self._sign(document)
        with self.assertRaisesRegex(RuntimeError, "only accepted for current profile 248"):
    ...
    def test_schema_tamper_is_rejected_before_dispatch(self) -> None:
        ...
        with (
            ...
            verify_gate(self.root, self.public_key, "248", accepted_schemas=frozenset({1, 2}))
    ...
    def test_signed_v1_is_rejected_by_v2_only_entry(self) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "schema is not accepted"):
            verify_gate(self.root, self.public_key, "248", accepted_schemas=frozenset({2}))
