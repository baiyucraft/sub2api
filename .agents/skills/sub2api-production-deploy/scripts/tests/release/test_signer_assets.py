...
from release.manifest import create_manifest, release_unit_relative_paths
...
class SignerAssetTest(unittest.TestCase):
    def test_bootstrap_installs_the_complete_release_unit(self) -> None:
        ...
        self.assertIn("for selftest_profile in 195 199 202 206 207 208 209 210 212 213 215 232 233 234 235 236 237 238 239 240 241 242 243 244 245 246 247 248 249 250 251 252 253 254 255; do", script)
    ...
    def test_manifest_binds_both_signers(self) -> None:
        ...
        release_assets = {
            units["validator"]: bootstrap.sha256_file(bootstrap.VALIDATOR),
            units["gate_signer"]: bootstrap.sha256_file(bootstrap.GATE_SIGNER),
            units["dr_signer"]: bootstrap.sha256_file(bootstrap.DR_SIGNER),
            units["space_cleaner"]: "a" * 64,
        }
        with (
            ...
            manifest = create_manifest("a" * 40, profile, "195-aaaaaaaaaaaa-1-aaaaaaaa", "blue-green")
    ...
    def test_dr_signer_contract_rejects_untrusted_shapes(self) -> None:
        ...
        self.assertIn("(195|199|202|206|207|208|209|210|212|213|215|232|233|234|235|236|237|238|239|240|241|242|243|244|245|246|247|248|249|250|251|252|253|254|255)", script)
    ...
    def test_backup_promotion_fixes_trust_and_candidate_contracts(self) -> None:
        ...
        supported = "(195|199|202|206|207|208|209|210|212|213|215|232|233|234|235|236|237|238|239|240|241|242|243|244|245|246|247|248|249|250|251|252|253|254|255)"
        self.assertIn(f"[[ $release_id =~ ^{supported}-", promoter)
        self.assertIn(f"[[ $drill_id =~ ^dr-{supported}-", promoter)
    ...
    def test_backup_asset_bootstrap_runs_negative_tests_before_activation(self) -> None:
        ...
        self.assertIn("for profile in 195 199 202 206 207 208 209 210 212 213 215 232 233 234 235 236 237 238 239 240 241 242 243 244 245 246 247 248 249 250 251 252 253 254 255; do", bootstrap_script)
