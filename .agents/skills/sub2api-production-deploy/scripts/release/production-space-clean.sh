...
write_candidate_archive_records() {
  ...
  while IFS= read -r -d '' path; do
    ...
    [[ -d $release_dir && ! -L $release_dir ]] || continue
    [[ $candidate_release =~ ^(182|187|191|192|194|195|197|198|199|202|206|207|208|209|210|212|213|215|232|233|234|235|236|237|238|239|240|241|242|243|244|245|246|247|248|249|250|251|252|253|254|255)-[0-9a-f]{12}-[0-9]+-[0-9a-f]{8}$ ]] || continue
    mtime=$(stat -c '%Y' "$path")
    size=$(stat -c '%s' "$path")
    links=$(stat -c '%h' "$path")
