...
# Legacy Gate v1 profile allowlist; Gate v2 current profile is 251 and 242-250 remain historical.
...
if [[ "$manifest_schema" == 2 ]]; then
  [[ "$profile" == 251 ]]
  [[ "$release_id" =~ ^251-[0-9a-f]{12}-[0-9]+-[0-9a-f]{8}$ ]]
  [[ "$version" == 0.2.4-baiyu ]]
  ...
  [[ $(jq -er '.parent_profile' "$manifest") == 250 ]]
