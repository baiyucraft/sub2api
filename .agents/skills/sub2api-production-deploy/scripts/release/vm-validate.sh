...
# Legacy Gate v1 profile allowlist; Gate v2 current profile is 247 and 242-246 remain historical.
...
if [[ "$manifest_schema" == 2 ]]; then
  [[ "$profile" == 247 ]]
  [[ "$release_id" =~ ^247-[0-9a-f]{12}-[0-9]+-[0-9a-f]{8}$ ]]
  [[ "$version" == 0.2.2-baiyu ]]
  ...
  [[ $(jq -er '.parent_profile' "$manifest") == 246 ]]
