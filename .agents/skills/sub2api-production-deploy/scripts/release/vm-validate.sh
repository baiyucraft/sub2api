...
gate_signing_failure_line=0
record_gate_signing_failure() {
  local exit_code=$?
  trap - ERR
  if [[ $exit_code -ne 0 ]]; then
    gate_signing_failure_line=${gate_signing_failure_line:-$LINENO}
    printf 'gate_signing_failure\n' > "$state_dir/failure-category.tmp"
    printf '%s\n' "$gate_signing_failure_line" > "$state_dir/failure-line.tmp"
...
trap 'gate_signing_failure_line=$LINENO; record_gate_signing_failure' ERR
