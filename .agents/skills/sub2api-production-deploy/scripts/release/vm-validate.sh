...
# Legacy Gate v1 profile allowlist; Gate v2 current profile is 246 and 242-245 remain historical.
...
if [[ "$manifest_schema" == 2 ]]; then
  [[ "$profile" == 246 ]]
  [[ "$release_id" =~ ^246-[0-9a-f]{12}-[0-9]+-[0-9a-f]{8}$ ]]
  [[ "$version" == 0.2.1-baiyu ]]
  ...
  [[ $(jq -er '.parent_profile' "$manifest") == 245 ]]
