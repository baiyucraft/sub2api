...
# Legacy Gate v1 profile allowlist; Gate v2 current profile is 250 and 242-249 remain historical.
...
if [[ "$manifest_schema" == 2 ]]; then
  [[ "$profile" == 250 ]]
  [[ "$release_id" =~ ^250-[0-9a-f]{12}-[0-9]+-[0-9a-f]{8}$ ]]
  [[ "$version" == 0.2.4-baiyu ]]
  ...
  [[ $(jq -er '.parent_profile' "$manifest") == 249 ]]
