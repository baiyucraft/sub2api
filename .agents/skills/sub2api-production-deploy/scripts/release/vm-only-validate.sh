...
[[ $scope == vm-only && $profile == 251 && $release_id == "251-${commit:0:12}-"* ]]
[[ $commit =~ ^[0-9a-f]{40}$ ]]
...
jq -cnS \
  ...
  '{gate_version:2,profile_id:251,manifest:$manifest,evidence:{candidate_image_id:$candidate_image_id,candidate_archive_sha256:$candidate_archive_sha256,candidate_size:$candidate_size,candidate_identity_verified:true,candidate_health:"pass",existing_app_health:"pass",vm_database_boundary:true,vm_redis_boundary:true,data_dev_boundary:true}}' >"$output_dir/gate.json"
