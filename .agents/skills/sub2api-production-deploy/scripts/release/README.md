...
```text
python .agents/skills/sub2api-production-deploy/scripts/release.py vm-only-validate --profile 252 --commit <40位完整SHA>
python .agents/skills/sub2api-production-deploy/scripts/release.py vm-only-switch --profile 252 --gate .tmp/releases/<release-id>/vm-only-gate
```
