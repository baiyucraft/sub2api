...
- SSH banner/EOF/transport 在建立连接阶段做有限重试；RackNerd 的纯只读 `doctor` 综合探针遇到连接中断或 `exit_code=-1` 时也可有限重放。RackNerd 经 SOCKS 代理下载大文件时使用单 SSH 连接、64 个有界预取请求并按本地文件长度续传；向 VM 上传大文件仍拆为 16 个可续传分片、最多同时建立 4 条 SSH 连接，并在并行失败后按未完成分片串行恢复。任何发布写命令已开始或状态不明时，仍必须先核对 release 目录、`.active-release`、committed marker 和 checksum，禁止自动重放。
