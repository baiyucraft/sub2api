...
```text
python .agents/skills/sub2api-production-deploy/scripts/release.py vm-only-validate --profile 250 --commit <40位完整SHA>
python .agents/skills/sub2api-production-deploy/scripts/release.py vm-only-switch --profile 250 --gate .tmp/releases/<release-id>/vm-only-gate
```
