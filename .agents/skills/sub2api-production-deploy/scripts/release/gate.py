...
def verify_gate(bundle_dir: Path, public_key: Path, expected_profile: str, allow_expired: bool = False, allow_historical_runner: bool = False) -> dict[str, Any]:
    ...
    if expected_profile in {"232", "233", "234", "235", "236", "237", "238", "239", "240", "241"}:
    ...
    if expected_profile in {"194", "195", "197", "198", "199", "202", "206", "207", "208", "209", "210", "212", "213", "215", "232", "233", "234", "235", "236", "237", "238", "239", "240", "241"} and evidence.get("prompt_audit_disabled") is not True:
        raise RuntimeError("gate lacks Prompt Audit disabled-state evidence")
    if expected_profile in {"195", "197", "198", "199", "202", "206", "207", "208", "209", "210", "212", "213", "215", "232", "233", "234", "235", "236", "237", "238", "239", "240", "241"}:
    ...
    if expected_profile in {"198", "199", "202", "206", "207", "208", "209", "210", "212", "213", "215", "232", "233", "234", "235", "236", "237", "238", "239", "240", "241"} and evidence.get("managed_monitor_key_names_verified") is not True:
        raise RuntimeError("gate lacks managed monitor key-name evidence")
    if expected_profile in {"199", "202", "206", "207", "208", "209", "210", "212", "213", "215", "232", "233", "234", "235", "236", "237", "238", "239", "240", "241"}:
    ...
    if expected_profile in {"202", "206", "207", "208", "209", "210", "212", "213", "215", "232", "233", "234", "235", "236", "237", "238", "239", "240", "241"}:
    ...
    if expected_profile in {"206", "207", "208", "209", "210", "212", "213", "215", "232", "233", "234", "235", "236", "237", "238", "239", "240", "241"}:
    ...
    if expected_profile in {"208", "209", "210", "212", "213", "215", "232", "233", "234", "235", "236", "237", "238", "239", "240", "241"} and evidence.get("passkey_schema_verified") is not True:
        raise RuntimeError("gate lacks profile 208 passkey schema evidence")
    if expected_profile in {"209", "210", "212", "213", "215", "232", "233", "234", "235", "236", "237", "238", "239", "240", "241"} and evidence.get("user_usage_aggregation_schema_verified") is not True:
        raise RuntimeError("gate lacks profile 209 user usage aggregation schema evidence")
    if expected_profile in {"212", "213", "215", "232", "233", "234", "235", "236", "237", "238", "239", "240", "241"}:
    ...
    if expected_profile in {"215", "232", "233", "234", "235", "236", "237", "238", "239", "240", "241"}:
    ...
    if expected_profile in {"233", "234", "235", "236", "237", "238", "239", "240", "241"}:
    ...
    if expected_profile in {"235", "236", "237", "238", "239", "240", "241"} and evidence.get("migration_234_status") not in {"absent", "verified"}:
        raise RuntimeError("gate lacks migration 234 status evidence")
    if expected_profile in {"235", "236", "237", "238", "239", "240", "241"} and evidence.get("migration_234_preflight_verified") is not True:
        raise RuntimeError("gate lacks profile 235 migration preflight evidence")
    if expected_profile in {"235", "236", "237", "238", "239", "240", "241"} and evidence.get("migration_234_schema_verified") is not True:
        raise RuntimeError("gate lacks profile 235 migration semantic evidence")
    if expected_profile in {"237", "238", "239", "240", "241"}:
        for migration_number in (235, 236):
            if evidence.get(f"migration_{migration_number}_status") not in {"absent", "verified"}:
                raise RuntimeError(f"gate lacks migration {migration_number} status evidence")
            if evidence.get(f"migration_{migration_number}_preflight_verified") is not True:
                raise RuntimeError(f"gate lacks migration {migration_number} preflight evidence")
            if evidence.get(f"migration_{migration_number}_schema_verified") is not True:
                raise RuntimeError(f"gate lacks migration {migration_number} semantic evidence")
    if expected_profile in {"238", "239", "240", "241"}:
        if evidence.get("migration_237_status") not in {"absent", "verified"}:
            raise RuntimeError("gate lacks migration 237 status evidence")
        if evidence.get("migration_237_preflight_verified") is not True:
            raise RuntimeError("gate lacks migration 237 preflight evidence")
        if evidence.get("migration_237_schema_verified") is not True:
            raise RuntimeError("gate lacks migration 237 semantic evidence")
    if expected_profile in {"239", "240", "241"}:
        if evidence.get("migration_238_status") not in {"absent", "verified"}:
            raise RuntimeError("gate lacks migration 238 status evidence")
        if evidence.get("migration_238_preflight_verified") is not True:
    ...
    if expected_profile in {"240", "241"}:
        for migration_number in (240, 241):
            if evidence.get(f"migration_{migration_number}_status") not in {"absent", "verified"}:
                raise RuntimeError(f"gate lacks migration {migration_number} status evidence")
            if evidence.get(f"migration_{migration_number}_preflight_verified") is not True:
                raise RuntimeError(f"gate lacks migration {migration_number} preflight evidence")
            if evidence.get(f"migration_{migration_number}_schema_verified") is not True:
                raise RuntimeError(f"gate lacks migration {migration_number} semantic evidence")
    if expected_profile == "241":
        for migration_number in (242, 243, 244, 245):
            if evidence.get(f"migration_{migration_number}_status") not in {"absent", "verified"}:
                raise RuntimeError(f"gate lacks migration {migration_number} status evidence")
            if evidence.get(f"migration_{migration_number}_preflight_verified") is not True:
                raise RuntimeError(f"gate lacks migration {migration_number} preflight evidence")
