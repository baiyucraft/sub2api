...
[[ $profile == 237 || $profile == 238 || $profile == 239 || $profile == 240 || $profile == 241 || $profile == 242 || $profile == 243 || $profile == 244 || $profile == 245 ]] || exit 1
...
if [[ $phase == preflight && $migration_status == absent ]]; then
  [[ $state == 0 ]] || exit 1
  printf 'migration_235_schema_state=absent\n'
else
  [[ $state == 2 ]] || exit 1
