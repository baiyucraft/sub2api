...
active_release_id=$(sed -n 's/^release_id=//p' "$active_slot_file")
[[ -z $active_release_id || $active_release_id =~ ^[A-Za-z0-9_.-]{1,128}$ ]]
...
printf 'container=%s\nport=%s\ninstance_id=%s\nrelease_id=%s\n' "$active_container" "$active_port" "$active_instance_id" "$active_release_id" > "$state_dir/pre-active-app"
