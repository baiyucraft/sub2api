...
- Native 页通过 `/admin/plugins/:pluginKey` 深链接加载，验证返回、插件管理侧栏高亮、停用态、定义损坏错误态、未保存草稿提醒、2～60 秒轮询暂停和 Action 幂等；Codex STATE 还必须验证中英文翻译、账号筛选/跨页选择、批量明确按钮、`result.config` 回填、日志表格和固定字段白名单；不得创建 iframe UI Session。旧 `ui/dist` 只作为 v1 iframe 回滚兼容路径验证。
...
VM Gate 或候选验证重放时，先按 migration 文件名和目标 checksum 读取数据库记录，再选择动作：

| 现场状态 | 允许动作 | 禁止动作 |
| --- | --- | --- |
| 记录不存在，目标 checksum 精确匹配 | 标记 `absent`，按 manifest 顺序执行 migration | 跳过执行或用旧 checksum 继续 |
...
以及 mismatch partial index 的列顺序、predicate、valid/ready 状态；profile 213 旧镜像必须沿
