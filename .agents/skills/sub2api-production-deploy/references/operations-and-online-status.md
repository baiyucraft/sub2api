...
- 生产维护清理另走 `cleanup-production`：记录 verified release、dry-run/apply、`plan_sha256`、full-SHA image/dangling layer 候选与删除数量、migration 证据容器数量、BuildKit policy/record 数和根/containerd 文件系统 `df` 差值。逻辑候选字节不得命名或解释为 reclaimable bytes。
