...
- 备份机空间清理只能使用版本化 `backup-retention-clean.sh`：先 dry-run，再用同一 `plan_sha256` apply。默认清理文件名时间早于 15 天且超过最新 1 组保护下限的 daily 加密包及其 checksum；`baseline`、`releases`、candidate、verified、恢复包和日志均不在自动清理范围。清理后仍低于 5 GiB 时必须扩容或人工审核，禁止扩大删除范围。
