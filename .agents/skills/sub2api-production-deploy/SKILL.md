...
- 发布身份必须是完整 40 位 commit SHA、唯一 full-SHA tag 和不可变 `candidate_image_id`；禁止用 `latest`、`main` 或短版本作为身份。
- 后端、数据库、迁移、fork/upstream、共享契约、配置语义、混合改动和不确定改动必须经过本地 `sub2api-dev`。
- 只有最终 diff 严格限定在 `frontend/`，且只包含 UI、样式、静态资源或前端测试时，才允许跳过 VM 门禁。
- 所有类别都走仓库根 `Dockerfile` 的完整后端发布流水线；禁止把本地 `frontend/dist` 直接复制到生产。
...
2. 按最终 diff 选择 `frontend-direct`、`dev-gated` 或 `build-chain`；不确定时从严。
3. 复用固定源码目录和依赖缓存，构建并记录 `candidate_image_id`。
4. 对需要 VM 门禁的改动，导入同一镜像、验证 VM 本地数据边界和容器行为。
5. 后端/数据库/fork/配置发布前执行生产备份门禁；不兼容迁移使用不会自动恢复写入的维护备份路径。
6. 只切换 `sub2api` 应用容器，完成 RackNerd 直连和 DMIT PROXY v2 双路径验收。
7. 生产通过后处理版本基线：`candidate -> 实际 PostgreSQL/Redis 隔离恢复 -> verified`。
