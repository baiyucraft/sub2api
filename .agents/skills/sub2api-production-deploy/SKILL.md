...
- Gate v2 当前 profile（现为 profile 254）的升级合同是 health-only；历史 profile 242-253 保留相同合同用于受控恢复/审计。只验证容器 health、应用 `/health` 和路由可达性；不得读取账号池、Canary/API key、Bearer 凭据，不发送模型/upstream 请求，不生成 usage attribution。`probe_*` 只有在明确表示隔离 DB/Redis/容器健康时才可使用。
