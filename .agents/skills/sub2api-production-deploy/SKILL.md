...
- 若备份目录已只剩最新 2 组 daily，而宿主机 systemd journal 占用异常，可使用版本化 `backup-host-space-clean.sh`：只将 `/var/log/journal` 容量压到 1 GiB，apply 必须绑定 dry-run 的 `plan_sha256`，并要求清理后至少保留 `5 GiB + 512 MiB` 上传余量。该脚本禁止删除、遍历或改写 `/srv/sub2api-backups` 内任何备份资产。
