...
- 备份机空间清理只能使用版本化 `backup-retention-clean.sh`：先 dry-run，再用同一 `plan_sha256` apply。默认清理文件名时间早于 15 天且超过最新 2 组保护下限的 daily 加密包及其 checksum；`baseline`、`releases`、candidate、verified、恢复包和日志均不在自动清理范围。清理后仍低于 5 GiB 时必须扩容或人工审核，禁止扩大删除范围。
- 若备份目录已无早于 15 天的 daily 自动清理候选且仍至少保留最新 2 组，而宿主机 systemd journal 占用异常，可使用版本化 `backup-host-space-clean.sh`：只将 `/var/log/journal` 容量压到 1 GiB，apply 必须绑定 dry-run 的 `plan_sha256`，并要求清理后至少保留 `5 GiB + 512 MiB` 上传余量。该脚本禁止删除、遍历或改写 `/srv/sub2api-backups` 内任何备份资产。
