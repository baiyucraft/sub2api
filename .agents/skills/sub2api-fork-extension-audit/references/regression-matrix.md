...
| 管理代理混合目录与兼容绑定 | 既有 `/admin/proxies`、`/admin/proxies/all` 默认返回真实代理和代理组虚拟行；真实行 `id>0,binding_type=proxy`，虚拟行 `id=-groupID,binding_type=proxy_ip_group,proxy_ip_group_id>0`；虚拟行不得进入真实代理操作；账号创建/更新的 `proxy_id>0` 校验真实代理，`proxy_id<0` 归一组字段，`proxy_id=0` 清空，归一化后再做类型/存在性/互斥校验；不新增路由 |
...
| migration/profile/version | migration 233 语义、官方 migration 编号冲突按内容重编号、历史 profile 233–254 合同不可变；254 保持 `0.2.7-baiyu`、parent 253、三项 278–280 原 migration；当前 255 为 `0.2.8-baiyu`、parent 254，`new_migrations=[281_content_moderation_engine_meta.sql,282_channel_reasoning_effort_multipliers.sql,283_affiliate_ledger_operation_id.sql]`，仅对官方原 238b/239/240 新文件重编号，SQL 字节不变；按完整 filename 排序，新库回填必须在历史 265 列创建后执行，升级库按 filename+checksum 保留原历史记录；新三项原始 SHA-256 与 catalog 核对，release manifest 绑定数据库 catalog 和兼容快照；执行 `backend/migrations/content_moderation_engine_meta_test.go`、`channel_reasoning_effort_multipliers_migration_test.go`、`backend/internal/repository/channel_reasoning_effort_migration_integration_test.go`、`affiliate_repo_test.go`；`VERSION = official release version + -baiyu`，每次升版新增连续 profile，不回写历史 |
