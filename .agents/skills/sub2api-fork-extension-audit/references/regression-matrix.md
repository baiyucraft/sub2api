...
| migration/profile/version | migration 233 语义、官方 migration 编号冲突按内容重编号、历史 profile 233–252 合同不可变；当前 profile 253 对应 `0.2.5-baiyu`、parent 为 252、`new_migrations=277_user_group_rate_percent.sql`，由 release manifest 绑定数据库 migration catalog 与生产兼容快照；用户专属倍率以 `rate_percent` 为业务真值，兼容绝对倍率按当前普通倍率派生；`VERSION = upstream VERSION + -baiyu`；fork VERSION 每变化一次都新增下一个连续 profile，不得回写旧 profile |
