...
type AccountQuery struct {
	...
	withProxyBindings  *AccountProxyBindingQuery
...
func (_q *AccountQuery) QueryProxies() *ProxyQuery {
	query := (&ProxyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryProxyBindings chains the current query on the "proxy_bindings" edge.
func (_q *AccountQuery) QueryProxyBindings() *AccountProxyBindingQuery {
	query := (&AccountProxyBindingClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withProxyBindings:  _q.withProxyBindings.Clone(),
...
func (_q *AccountQuery) WithProxies(opts ...func(*ProxyQuery)) *AccountQuery {
	query := (&ProxyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithProxyBindings tells the query-builder to eager-load the nodes that are connected to
// the "proxy_bindings" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AccountQuery) WithProxyBindings(opts ...func(*AccountProxyBindingQuery)) *AccountQuery {
	query := (&AccountProxyBindingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withProxyBindings = query
	return _q
}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withProxies; query != nil {
		if err := _q.loadProxies(ctx, query, nodes,
			func(n *Account) { n.Edges.Proxies = []*Proxy{} },
			func(n *Account, e *Proxy) { n.Edges.Proxies = append(n.Edges.Proxies, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withProxyBindings; query != nil {
		if err := _q.loadProxyBindings(ctx, query, nodes,
			func(n *Account) { n.Edges.ProxyBindings = []*AccountProxyBinding{} },
			func(n *Account, e *AccountProxyBinding) { n.Edges.ProxyBindings = append(n.Edges.ProxyBindings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AccountQuery) loadProxies(ctx context.Context, query *ProxyQuery, nodes []*Account, init func(*Account), assign func(*Account, *Proxy)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Account)
	nids := make(map[int64]map[*Account]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(account.ProxiesTable)
		s.Join(joinT).On(s.C(proxy.FieldID), joinT.C(account.ProxiesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(account.ProxiesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(account.ProxiesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *AccountQuery) loadProxyBindings(ctx context.Context, query *AccountProxyBindingQuery, nodes []*Account, init func(*Account), assign func(*Account, *AccountProxyBinding)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(accountproxybinding.FieldAccountID)
	}
	query.Where(predicate.AccountProxyBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.ProxyBindingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
