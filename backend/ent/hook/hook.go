...
// The AccountProxyBindingFunc type is an adapter to allow the use of ordinary
// function as AccountProxyBinding mutator.
type AccountProxyBindingFunc func(context.Context, *ent.AccountProxyBindingMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AccountProxyBindingFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AccountProxyBindingMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AccountProxyBindingMutation", m)
}
