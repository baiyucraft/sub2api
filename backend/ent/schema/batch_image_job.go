...
func (BatchImageJob) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int64("upstream_config_id").Optional().Nillable(),
		field.Int64("upstream_key_id").Optional().Nillable(),
		field.String("upstream_cost_currency").MaxLen(8).Optional().Nillable(),
		field.Float("upstream_cost_to_cny_rate").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)"}),
