...
type APIKey struct {
	...
	// Per-key scheduling preference: preserve affinity or enable session-level switching.
	SchedulingMode apikey.SchedulingMode `json:"scheduling_mode,omitempty"`
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case apikey.FieldKey, apikey.FieldName, apikey.FieldPurpose, apikey.FieldStatus, apikey.FieldSchedulingMode:
...
func (_m *APIKey) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case apikey.FieldSchedulingMode:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field scheduling_mode", values[i])
			} else if value.Valid {
				_m.SchedulingMode = apikey.SchedulingMode(value.String)
			}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("scheduling_mode=")
	builder.WriteString(fmt.Sprintf("%v", _m.SchedulingMode))
	builder.WriteString(", ")
