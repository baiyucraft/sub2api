...
type APIKeyQuery struct {
	...
	withManagedChannelMonitors *ChannelMonitorQuery
...
// QueryManagedChannelMonitors chains the current query on the "managed_channel_monitors" edge.
func (_q *APIKeyQuery) QueryManagedChannelMonitors() *ChannelMonitorQuery {
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withManagedChannelMonitors: _q.withManagedChannelMonitors.Clone(),
...
// WithManagedChannelMonitors tells the query-builder to eager-load the nodes that are connected to
// the "managed_channel_monitors" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithManagedChannelMonitors(opts ...func(*ChannelMonitorQuery)) *APIKeyQuery {
	query := (&ChannelMonitorClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withManagedChannelMonitors = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			_q.withManagedChannelMonitors != nil,
		}
	)
	...
	if query := _q.withManagedChannelMonitors; query != nil {
		if err := _q.loadManagedChannelMonitors(ctx, query, nodes,
			func(n *APIKey) { n.Edges.ManagedChannelMonitors = []*ChannelMonitor{} },
			func(n *APIKey, e *ChannelMonitor) {
				n.Edges.ManagedChannelMonitors = append(n.Edges.ManagedChannelMonitors, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadManagedChannelMonitors(ctx context.Context, query *ChannelMonitorQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *ChannelMonitor)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(channelmonitor.FieldManagedAPIKeyID)
	}
	query.Where(predicate.ChannelMonitor(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.ManagedChannelMonitorsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.ManagedAPIKeyID
		if fk == nil {
			return fmt.Errorf(`foreign-key "managed_api_key_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "managed_api_key_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
