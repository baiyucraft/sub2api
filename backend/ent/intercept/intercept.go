...
// The AccountProxyBindingFunc type is an adapter to allow the use of ordinary function as a Querier.
type AccountProxyBindingFunc func(context.Context, *ent.AccountProxyBindingQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f AccountProxyBindingFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.AccountProxyBindingQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.AccountProxyBindingQuery", q)
}

// The TraverseAccountProxyBinding type is an adapter to allow the use of ordinary function as Traverser.
type TraverseAccountProxyBinding func(context.Context, *ent.AccountProxyBindingQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseAccountProxyBinding) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseAccountProxyBinding) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.AccountProxyBindingQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.AccountProxyBindingQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.AccountProxyBindingQuery:
		return &query[*ent.AccountProxyBindingQuery, predicate.AccountProxyBinding, accountproxybinding.OrderOption]{typ: ent.TypeAccountProxyBinding, tq: q}, nil
