...
type ProxyQuery struct {
	...
	withProxyIPGroupMembers *ProxyIPGroupMemberQuery
...
// QueryProxyIPGroups chains the current query on the "proxy_ip_groups" edge.
func (_q *ProxyQuery) QueryProxyIPGroups() *ProxyIPGroupQuery {
...
// QueryProxyIPGroupMembers chains the current query on the "proxy_ip_group_members" edge.
func (_q *ProxyQuery) QueryProxyIPGroupMembers() *ProxyIPGroupMemberQuery {
...
func (_q *ProxyQuery) Clone() *ProxyQuery {
	...
	return &ProxyQuery{
		...
		withProxyIPGroupMembers: _q.withProxyIPGroupMembers.Clone(),
...
// WithProxyIPGroups tells the query-builder to eager-load the nodes that are connected to
// the "proxy_ip_groups" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ProxyQuery) WithProxyIPGroups(opts ...func(*ProxyIPGroupQuery)) *ProxyQuery {
	query := (&ProxyIPGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withProxyIPGroups = query
	return _q
}

// WithProxyIPGroupMembers tells the query-builder to eager-load the nodes that are connected to
// the "proxy_ip_group_members" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ProxyQuery) WithProxyIPGroupMembers(opts ...func(*ProxyIPGroupMemberQuery)) *ProxyQuery {
	query := (&ProxyIPGroupMemberClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withProxyIPGroupMembers = query
	return _q
}
...
func (_q *ProxyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Proxy, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			_q.withProxyIPGroupMembers != nil,
		}
	)
	...
	if query := _q.withProxyIPGroups; query != nil {
		if err := _q.loadProxyIPGroups(ctx, query, nodes,
			func(n *Proxy) { n.Edges.ProxyIPGroups = []*ProxyIPGroup{} },
			func(n *Proxy, e *ProxyIPGroup) { n.Edges.ProxyIPGroups = append(n.Edges.ProxyIPGroups, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withProxyIPGroupMembers; query != nil {
		if err := _q.loadProxyIPGroupMembers(ctx, query, nodes,
			func(n *Proxy) { n.Edges.ProxyIPGroupMembers = []*ProxyIPGroupMember{} },
			func(n *Proxy, e *ProxyIPGroupMember) {
				n.Edges.ProxyIPGroupMembers = append(n.Edges.ProxyIPGroupMembers, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *ProxyQuery) loadProxyIPGroups(ctx context.Context, query *ProxyIPGroupQuery, nodes []*Proxy, init func(*Proxy), assign func(*Proxy, *ProxyIPGroup)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(proxy.ProxyIPGroupsTable)
		s.Join(joinT).On(s.C(proxyipgroup.FieldID), joinT.C(proxy.ProxyIPGroupsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(proxy.ProxyIPGroupsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(proxy.ProxyIPGroupsPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*ProxyIPGroup](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "proxy_ip_groups" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *ProxyQuery) loadProxyIPGroupMembers(ctx context.Context, query *ProxyIPGroupMemberQuery, nodes []*Proxy, init func(*Proxy), assign func(*Proxy, *ProxyIPGroupMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Proxy)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(proxyipgroupmember.FieldProxyID)
	}
	query.Where(predicate.ProxyIPGroupMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(proxy.ProxyIPGroupMembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
