...
func RegisterAdminRoutes(
	...
	admin.Use(middleware.AdminComplianceGuard(settingService))
	{
		...
		registerProxyIPGroupRoutes(admin, h)
...
func registerUpstreamManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	group.GET("/settings", h.Admin.UpstreamConfig.GetUpstreamManagementSettings)
	group.PUT("/settings", h.Admin.UpstreamConfig.PutUpstreamManagementSettings)
...
func registerDashboardRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	dashboard := admin.Group("/dashboard")
	{
		...
		dashboard.POST("/users-usage", middleware.RequestBodyLimit(64<<10), h.Admin.Dashboard.GetBatchUsersUsage)
...
func registerGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	groups := admin.Group("/groups")
	{
		...
		groups.POST("/quality-stats/batch", middleware.RequestBodyLimit(64<<10), h.Admin.Group.GetBatchQualityStats)
		groups.GET("/live-capability", h.Admin.Group.GetLiveCapability)
		groups.GET("/ttft-guard-policies", h.Admin.Group.ListTTFTGuardPolicies)
		groups.PUT("/sort-order", h.Admin.Group.UpdateSortOrder)
		groups.GET("/:id/ttft-guard-policy", h.Admin.Group.GetTTFTGuardPolicy)
		groups.PUT("/:id/ttft-guard-policy", h.Admin.Group.PutTTFTGuardPolicy)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/quality-stats/batch", middleware.RequestBodyLimit(64<<10), h.Admin.Account.GetBatchQualityStats)
...
func registerUpstreamConfigRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	upstreams := admin.Group("/upstream-configs")
	{
		...
		upstreams.GET("/:id/auth-session", h.Admin.UpstreamConfig.GetAuthSession)
		upstreams.DELETE("/:id/auth-session", h.Admin.UpstreamConfig.ClearAuthSession)
		upstreams.POST("/:id/auth-session/clear-cooldown", h.Admin.UpstreamConfig.ClearAuthSessionCooldown)
		upstreams.POST("/:id/auth-session/force-reauth", h.Admin.UpstreamConfig.ForceAuthSessionReauth)
		...
		upstreams.PUT("/:id/keys/:keyID/platform", h.Admin.UpstreamConfig.UpdateKeyPlatform)
		upstreams.PUT("/:id/keys/:keyID/base-url", h.Admin.UpstreamConfig.UpdateKeyBaseURL)
...
func registerProxyIPGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	groups := admin.Group("/proxy-ip-groups")
	{
		groups.GET("", h.Admin.ProxyIPGroup.List)
		groups.GET("/:id", h.Admin.ProxyIPGroup.GetByID)
		groups.POST("", h.Admin.ProxyIPGroup.Create)
		groups.PUT("/:id", h.Admin.ProxyIPGroup.Update)
		groups.PUT("/:id/members", h.Admin.ProxyIPGroup.UpdateMembers)
		groups.DELETE("/:id", h.Admin.ProxyIPGroup.Delete)
	}
}
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		adminSettings.GET("/openai-ttft-guard", h.Admin.Setting.GetOpenAITTFTGuardSettings)
		adminSettings.PUT("/openai-ttft-guard", h.Admin.Setting.UpdateOpenAITTFTGuardSettings)
