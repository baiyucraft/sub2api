...
func TestPrepareUsageLogInsert_ArgCountMatchesTypes(t *testing.T) {
	...
	require.Len(t, prepared.args, 66) // fork attribution fields plus upstream_request_id
	...
	require.Equal(t, 17, prepared.args[21])
	require.Equal(t, 0.25, prepared.args[22])
	require.Equal(t, sql.NullString{String: upstreamCostCurrency, Valid: true}, prepared.args[31])
	require.Equal(t, &upstreamCostToCNYRate, prepared.args[32])
}
...
func TestUsageLogRepositoryGetUserUsageTrendScansAccountCost(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	...
	rows := sqlmock.NewRows([]string{
		"date", "user_id", "email", "username", "requests", "tokens", "cost", "actual_cost", "account_cost",
	}).AddRow("2025-01-01 00:00", int64(7), "user@example.com", "user", int64(3), int64(900), 1.5, 1.2, 0.8)
