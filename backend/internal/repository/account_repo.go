...
func createAccountRecord(ctx context.Context, client *dbent.Client, account *service.Account) (*dbent.Account, error) {
	if account == nil {
		return nil, service.ErrAccountNilInput
	}
	if err := validateUpstreamAccountBindingForWrite(ctx, client, account); err != nil {
		return nil, err
	}
...
func (r *accountRepository) CreateWithAccountGroups(ctx context.Context, account *service.Account, groups []service.AccountGroup) error {
	...
	err := r.withAccountWriteTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		...
		persistedGroups = make([]service.AccountGroup, len(groups))
		if len(groups) > 0 {
			builders := make([]*dbent.AccountGroupCreate, 0, len(groups))
			for i := range groups {
				persistedGroups[i] = groups[i]
				persistedGroups[i].AccountID = created.ID
		...
		return enqueueSchedulerOutbox(txCtx, txClient, service.SchedulerOutboxEventAccountChanged, &created.ID, nil, buildSchedulerGroupPayload(groupIDs))
	})
	if err != nil {
		return err
	}
	...
	account.AccountGroups = persistedGroups
	return nil
}
...
func (r *accountRepository) updateLockedAccount(
	...
	if err := lockAndValidateUpstreamAccountUpdate(ctx, client, account); err != nil {
		return nil, err
	}
	...
	updated, err := buildAccountUpdate(client, account, schedulable, explicitRateMultiplier).Save(ctx)
	if err != nil {
		return nil, translatePersistenceError(err, service.ErrAccountNotFound, nil)
	}
	return updated, nil
...
func buildAccountUpdate(client *dbent.Client, account *service.Account, schedulable bool, explicitRateMultiplier *float64) *dbent.AccountUpdateOne {
...
func lockAndMergeAccountProbeExtra(
	...
	probeAccount := service.IsUpstreamBillingProbeIdentity(account.Platform, account.Type) && !account.IsUpstreamBound()
	probeEnabled := false
	...
	if probeAccount && identityUnchanged && !probeExplicitlyDisabled {
...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []int64, updates service.AccountBulkUpdate) (int64, error) {
	...
	idx++
	if updates.ProbeEnabled != nil {
		whereClause += " AND type = $" + itoa(idx) + " AND upstream_config_id IS NULL AND upstream_key_id IS NULL"
		args = append(args, service.AccountTypeAPIKey)
	}
	query := "UPDATE accounts SET " + joinClauses(setClauses, ", ") + whereClause
	expectedProbeRows := int64(0)
	if updates.ProbeEnabled != nil {
	...
	var err error
	if r.client == nil {
		result, execErr := r.sql.ExecContext(ctx, query, args...)
		if execErr != nil {
			return 0, execErr
		}
		...
		err = r.withAccountWriteTx(ctx, func(txCtx context.Context, client *dbent.Client) error {
			if lockErr := lockBulkUpdateUpstreamReferences(txCtx, client, ids); lockErr != nil {
				return lockErr
			}
	...
	rows, err := r.sql.QueryContext(ctx, `
		WITH candidates AS (
			...
			WHERE deleted_at IS NULL
				AND status = 'active'
				AND type = 'apikey'
				AND upstream_config_id IS NULL
				AND upstream_key_id IS NULL
				AND extra @> '{"upstream_billing_probe_enabled": true}'::jsonb
