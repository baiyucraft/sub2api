...
func (r *userGroupRateRepository) withTransaction(ctx context.Context, fn func(*sql.Tx) error) error {
...
// GetByUserID 获取用户所有专属分组的当前有效倍率。
func (r *userGroupRateRepository) GetByUserID(ctx context.Context, userID int64) (map[int64]float64, error) {
	query := `
		SELECT ugr.group_id,
		       COALESCE(g.rate_multiplier * ugr.rate_percent / 100.0, ugr.rate_multiplier)::double precision
		FROM user_group_rate_multipliers ugr
		JOIN groups g ON g.id = ugr.group_id
		WHERE ugr.user_id = $1
		  AND (ugr.rate_percent IS NOT NULL OR ugr.rate_multiplier IS NOT NULL)
	`
...
// GetPercentByUserID 获取用户所有专属分组的百分比真值。
func (r *userGroupRateRepository) GetPercentByUserID(ctx context.Context, userID int64) (map[int64]float64, error) {
	rows, err := r.sql.QueryContext(ctx, `
		SELECT ugr.group_id,
		       COALESCE(ugr.rate_percent, ugr.rate_multiplier / g.rate_multiplier * 100.0)::double precision
		FROM user_group_rate_multipliers ugr
		JOIN groups g ON g.id = ugr.group_id
		WHERE ugr.user_id = $1
		  AND (ugr.rate_percent IS NOT NULL OR ugr.rate_multiplier IS NOT NULL)
	...
	for rows.Next() {
		var groupID int64
		var percent float64
		if err := rows.Scan(&groupID, &percent); err != nil {
			return nil, err
		}
		result[groupID] = percent
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
...
// GetPercentsByUserIDs 批量获取多个用户的专属分组百分比真值。
func (r *userGroupRateRepository) GetPercentsByUserIDs(ctx context.Context, userIDs []int64) (map[int64]map[int64]float64, error) {
	result := make(map[int64]map[int64]float64, len(userIDs))
	if len(userIDs) == 0 {
		return result, nil
	}
	...
	for rows.Next() {
		...
		var groupID int64
		var percent float64
		if err := rows.Scan(&userID, &groupID, &percent); err != nil {
			return nil, err
		}
		if _, ok := result[userID]; !ok {
			result[userID] = make(map[int64]float64)
		}
		result[userID][groupID] = percent
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
...
func (r *userGroupRateRepository) GetByGroupID(ctx context.Context, groupID int64) ([]service.UserGroupRateEntry, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&entry.UserID, &entry.UserName, &entry.UserEmail, &entry.UserNotes, &entry.UserStatus, &rate, &percent, &rpm); err != nil {
			return nil, err
		}
		if rate.Valid {
			v := rate.Float64
...
// GetByUserAndGroup 获取用户在特定分组的当前有效倍率（NULL 返回 nil）。
func (r *userGroupRateRepository) GetByUserAndGroup(ctx context.Context, userID, groupID int64) (*float64, error) {
	query := `
		SELECT COALESCE(g.rate_multiplier * ugr.rate_percent / 100.0, ugr.rate_multiplier)::double precision
		FROM user_group_rate_multipliers ugr
		JOIN groups g ON g.id = ugr.group_id
		WHERE ugr.user_id = $1 AND ugr.group_id = $2
	`
	var rate sql.NullFloat64
...
// GetPercentByUserAndGroup 获取用户在特定分组的专属百分比（NULL 返回 nil）。
func (r *userGroupRateRepository) GetPercentByUserAndGroup(ctx context.Context, userID, groupID int64) (*float64, error) {
	query := `
		SELECT COALESCE(ugr.rate_percent, ugr.rate_multiplier / g.rate_multiplier * 100.0)::double precision
		FROM user_group_rate_multipliers ugr
		JOIN groups g ON g.id = ugr.group_id
		WHERE ugr.user_id = $1 AND ugr.group_id = $2
	`
	var percent sql.NullFloat64
	err := scanSingleRow(ctx, r.sql, query, []any{userID, groupID}, &percent)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
	...
	return r.withTransaction(ctx, func(tx *sql.Tx) error {
		...
		`, userID); err != nil {
			...
			_, err := tx.ExecContext(ctx,
				`DELETE FROM user_group_rate_multipliers WHERE user_id = $1 AND rate_percent IS NULL AND rate_multiplier IS NULL AND rpm_override IS NULL`,
				userID)
			return err
		}
		...
		`, userID, pq.Array(clearGroupIDs)); err != nil {
			...
			if _, err := tx.ExecContext(ctx,
				`DELETE FROM user_group_rate_multipliers WHERE user_id = $1 AND group_id = ANY($2) AND rate_percent IS NULL AND rate_multiplier IS NULL AND rpm_override IS NULL`,
				userID, pq.Array(clearGroupIDs)); err != nil {
				return err
			}
		}
...
// SyncUserGroupRatePercents 同步用户的分组专属百分比。
// nil 表示清除；非 nil 值由数据库触发器同步兼容 rate_multiplier 影子列。
func (r *userGroupRateRepository) SyncUserGroupRatePercents(ctx context.Context, userID int64, percents map[int64]*float64) error {
	return r.withTransaction(ctx, func(tx *sql.Tx) error {
		if len(percents) == 0 {
			if _, err := tx.ExecContext(ctx, `
			UPDATE user_group_rate_multipliers
			SET rate_percent = NULL, updated_at = NOW()
			WHERE user_id = $1
		`, userID); err != nil {
				return err
			}
		...
		var clearGroupIDs []int64
		upsertGroupIDs := make([]int64, 0, len(percents))
		upsertPercents := make([]float64, 0, len(percents))
		for groupID, percent := range percents {
			if percent == nil {
				clearGroupIDs = append(clearGroupIDs, groupID)
			} else {
				upsertGroupIDs = append(upsertGroupIDs, groupID)
				upsertPercents = append(upsertPercents, *percent)
			}
		}
		...
		if len(upsertGroupIDs) > 0 {
			now := time.Now()
			_, err := tx.ExecContext(ctx, `
			INSERT INTO user_group_rate_multipliers (user_id, group_id, rate_percent, created_at, updated_at)
			SELECT
				$1::bigint,
				data.group_id,
				data.rate_percent,
				$2::timestamptz,
				$2::timestamptz
			FROM unnest($3::bigint[], $4::double precision[]) AS data(group_id, rate_percent)
			ON CONFLICT (user_id, group_id)
			DO UPDATE SET
				rate_percent = EXCLUDED.rate_percent,
				updated_at = EXCLUDED.updated_at
		`, userID, now, pq.Array(upsertGroupIDs), pq.Array(upsertPercents))
			if err != nil {
				return err
			}
		}
...
// SyncGroupRateMultipliers 同步分组的 rate_percent 部分（不触动 rpm_override）。
// 语义：
...
func (r *userGroupRateRepository) SyncGroupRateMultipliers(ctx context.Context, groupID int64, entries []service.GroupRateMultiplierInput) error {
	return r.withTransaction(ctx, func(tx *sql.Tx) error {
		...
		// 未在 entries 列表中的行：清空 rate_percent；触发器同步清空兼容影子列。
		...
		WHERE group_id = $1 AND rate_percent IS NULL AND rate_multiplier IS NULL AND rpm_override IS NULL
	`, groupID); err != nil {
			return err
		}
		...
		percents := make([]float64, len(entries))
		for i, e := range entries {
			userIDs[i] = e.UserID
			percents[i] = *e.RatePercent
		}
		now := time.Now()
		_, err := tx.ExecContext(ctx, `
		INSERT INTO user_group_rate_multipliers (user_id, group_id, rate_percent, created_at, updated_at)
		SELECT data.user_id, $1::bigint, data.rate_percent, $2::timestamptz, $2::timestamptz
		FROM unnest($3::bigint[], $4::double precision[]) AS data(user_id, rate_percent)
		ON CONFLICT (user_id, group_id)
		DO UPDATE SET rate_percent = EXCLUDED.rate_percent, updated_at = EXCLUDED.updated_at
	`, groupID, now, pq.Array(userIDs), pq.Array(percents))
		return err
	})
}
