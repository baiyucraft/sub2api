...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	if filters.UpstreamConfigID > 0 {
		conditions = append(conditions, fmt.Sprintf("upstream_config_id = $%d", len(args)+1))
		args = append(args, filters.UpstreamConfigID)
	}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log := &service.UsageLog{
		...
		APIKeyID:                  apiKeyID,
		AccountID:                 accountID,
		UpstreamConfigID:          usageLogNullInt64Ptr(upstreamConfigID),
		UpstreamKeyID:             usageLogNullInt64Ptr(upstreamKeyID),
		Model:                     model,
