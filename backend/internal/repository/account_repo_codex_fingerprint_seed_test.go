...
func TestBulkUpdateCodexFingerprintSeedRollsBackWhenUpdateFails(t *testing.T) {
	...
	mock.ExpectBegin()
	accountRows := sqlmock.NewRows([]string{"id", "upstream_config_id", "upstream_key_id"})
	accountRows.AddRow(int64(27), nil, nil).AddRow(int64(28), nil, nil)
	mock.ExpectQuery(`SELECT .* FROM "accounts"`).WillReturnRows(accountRows)
	secondAccountRows := sqlmock.NewRows([]string{"id", "upstream_config_id", "upstream_key_id"})
	secondAccountRows.AddRow(int64(27), nil, nil).AddRow(int64(28), nil, nil)
	mock.ExpectQuery(`SELECT .* FROM "accounts"`).WillReturnRows(secondAccountRows)
...
func TestBulkUpdateCodexFingerprintSeedRollsBackWhenOutboxFails(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectQuery(`SELECT .* FROM "accounts"`).WillReturnRows(sqlmock.NewRows([]string{"id", "upstream_config_id", "upstream_key_id"}).
		AddRow(int64(27), nil, nil).AddRow(int64(28), nil, nil))
	mock.ExpectQuery(`SELECT .* FROM "accounts"`).WillReturnRows(sqlmock.NewRows([]string{"id", "upstream_config_id", "upstream_key_id"}).
		AddRow(int64(27), nil, nil).AddRow(int64(28), nil, nil))
	mock.ExpectExec(`(?s)UPDATE accounts SET extra = .*gen_random_uuid\(\)::text.*WHERE id = ANY\(\$2\)`).
		WithArgs(sqlmock.AnyArg(), `{27,28}`).
