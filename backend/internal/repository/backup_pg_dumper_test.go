...
func fixtureDumpCmd(ctx context.Context, stdout string, exitCode int) *exec.Cmd {
	if runtime.GOOS == "windows" {
		script := fmt.Sprintf("[Console]::Out.Write('%s'); exit %d", stdout, exitCode)
		return exec.CommandContext(ctx, "powershell", "-NoProfile", "-NonInteractive", "-Command", script)
	}
	script := fmt.Sprintf("printf %s; exit %d", stdout, exitCode)
	return exec.CommandContext(ctx, "sh", "-c", script)
}

func fixtureTrueCmd(ctx context.Context) *exec.Cmd {
	if runtime.GOOS == "windows" {
...
func TestPgDumperHoldsMigrationLockThroughReaderClose(t *testing.T) {
	...
	dumper, createdMock := newTestPgDumper(t, func(ctx context.Context, name string, args ...string) *exec.Cmd {
		...
		return fixtureDumpCmd(ctx, "backup-data", 0)
	})
	mock = createdMock
...
func TestPgDumperReleasesMigrationLockWhenProcessFails(t *testing.T) {
	dumper, mock := newTestPgDumper(t, func(ctx context.Context, _ string, _ ...string) *exec.Cmd {
		return fixtureDumpCmd(ctx, "partial-backup", 7)
	})
	expectBackupMigrationLock(mock)
