...
// purpose 与 managed_monitor_id 只允许由托管监控仓储维护，普通 API Key
// 更新即使拿到被篡改的陈旧快照，也不能改写这两个归属字段。
func (s *APIKeyRepoSuite) TestUpdate_DoesNotModifyManagedMonitorOwnershipFields() {
	user := s.mustCreateUser("apikey-managed-fields@example.com")
	monitorID := int64(12345)
	key := &service.APIKey{
		UserID:  user.ID,
		Key:     "sk-managed-fields",
		Name:    "before",
	...
	s.Require().NoError(err, "GetByID")
	stale.Name = "after"
	stale.Purpose = service.APIKeyPurposeManagedMonitor
	stale.ManagedMonitorID = &monitorID
	...
	got, err := s.repo.GetByID(s.ctx, key.ID)
	s.Require().NoError(err, "GetByID after update")
	s.Require().Equal("after", got.Name)
	s.Require().Equal(service.APIKeyPurposeGeneral, got.Purpose)
	s.Require().Nil(got.ManagedMonitorID)
}
