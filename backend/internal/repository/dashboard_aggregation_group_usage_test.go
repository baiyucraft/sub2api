...
func TestDashboardAggregationRepositoryCleanupUsageLogsNonPartitionedInvalidatesEachBatchAndSyncs(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT status, coverage_start, coverage_end, target_end`).
		WillReturnRows(sqlmock.NewRows([]string{"status", "coverage_start", "coverage_end", "target_end"}).
			AddRow(userUsageBackfillAvailable, cutoff.AddDate(0, -3, 0), cutoff.Add(time.Hour), nil))
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT MIN(created_at), MAX(created_at)`)).
		WithArgs(cutoff).
		WillReturnRows(sqlmock.NewRows([]string{"min", "max"}).AddRow(earliestDeletedAt, earliestDeletedAt.Add(time.Hour)))
	mock.ExpectQuery(`SELECT id FROM usage_group_rollup_state.*FOR UPDATE`).
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(1))
...
func TestDashboardAggregationRepositoryCleanupUsageLogsNonPartitionedFailureRollsBackWithoutSync(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT MIN(created_at), MAX(created_at)`)).
		WithArgs(cutoff).
		WillReturnRows(sqlmock.NewRows([]string{"min", "max"}).AddRow(deletedAt, deletedAt.Add(time.Hour)))
	mock.ExpectQuery(`SELECT id FROM usage_group_rollup_state.*FOR UPDATE`).
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(1))
