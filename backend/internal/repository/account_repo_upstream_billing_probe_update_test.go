...
func TestLockAndMergeAccountProbeExtraKeepsOllamaManagedFieldsForUpstreamBoundAccount(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
		...
		WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "enabled", "rate_sync_enabled", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot", "opencode_group_unchanged", "opencode_auto", "opencode_snapshot", "current_extra"}).
			AddRow(true, true, true, []byte(`true`), []byte(`true`), []byte(`{"status":"probe"}`), []byte(`"local-ciphertext"`), []byte(`true`), []byte(`{"status":"ok"}`), false, nil, nil, nil))
...
func TestBulkUpdateProbeEligibilityMismatchRollsBack(t *testing.T) {
	...
	enabled := true
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)SELECT .* FROM "accounts".*"id" IN.*ORDER BY .*ASC`).
		WithArgs(int64(27), int64(28)).
		WillReturnRows(updatedAccountRowsForIDs(`{}`, 27, 28))
	mock.ExpectQuery(`(?s)SELECT .* FROM "accounts".*"id" IN.*ORDER BY .*ASC.*FOR UPDATE`).
		WithArgs(int64(27), int64(28)).
		WillReturnRows(updatedAccountRowsForIDs(`{}`, 27, 28))
	mock.ExpectExec(`(?s)UPDATE accounts SET extra = .* WHERE id = ANY\(\$2\) AND deleted_at IS NULL AND type = \$3 AND upstream_config_id IS NULL AND upstream_key_id IS NULL`).
...
func updatedAccountRows(id int64, extra string) *sqlmock.Rows {
	return updatedAccountRowsForIDs(extra, id)
}

func updatedAccountRowsForIDs(extra string, ids ...int64) *sqlmock.Rows {
	now := time.Now()
	rows := sqlmock.NewRows(dbaccount.Columns)
	for _, id := range ids {
		rows.AddRow(
			id, now, now, nil, "test", nil, service.PlatformOpenAI, service.AccountTypeAPIKey,
			[]byte(`{"api_key":"sk-test"}`), []byte(extra), nil, nil, nil, nil, nil, nil, nil, "manual", nil, 1, 0, 0, nil, 1, 1.0, nil,
