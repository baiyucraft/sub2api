...
func TestSchedulerCacheSetSnapshotExcludesUnencodableAccounts(t *testing.T) {
	ctx := context.Background()
	cache := newSchedulerCacheUnit(t)
	invalidTime := time.Date(10000, time.January, 1, 0, 0, 0, 0, time.UTC)
	bucket := service.SchedulerBucket{Platform: service.PlatformOpenAI, GroupID: 9}
	token, err := cache.CaptureBucketWriteToken(ctx, bucket)
	require.NoError(t, err)

	require.NoError(t, cache.SetSnapshot(ctx, bucket, token, []service.Account{
		{ID: 121, Platform: service.PlatformOpenAI, Type: service.AccountTypeAPIKey},
		{ID: 122, Platform: service.PlatformOpenAI, Type: service.AccountTypeAPIKey, ExpiresAt: &invalidTime},
	}))
	...
	require.Equal(t, int64(121), snapshot[0].ID)
}
