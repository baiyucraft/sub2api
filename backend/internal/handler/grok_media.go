...
// prepareGrokVideoCompletionBilling builds a durable billing event for official
// done+video.url observations. The database usage_billing_dedup transaction is
// the authoritative one-shot claim; no Redis claim is taken before the durable
// write, so a process exit cannot strand a completed job as already billed.
...
func recordGrokMediaUsage(
	...
	// Async video: force a durable task request id. The database billing
	// transaction owns idempotency, so every observation may safely retry it.
	videoTaskID := ""
	if result != nil && result.VideoCount > 0 {
		...
		// Always use the task id for fingerprint stability across status/content
		// observations; their response/request bodies are intentionally different.
