...
func AccountFromServiceShallow(a *service.Account) *Account {
	...
	out := &Account{
		...
		UpstreamConfigName:        a.UpstreamConfigName,
		UpstreamKeyName:           a.UpstreamKeyName,
		UpstreamKeyMasked:         a.UpstreamKeyMasked,
		UpstreamSchedulingEnabled: a.UpstreamSchedulingEnabled,
		Concurrency:               a.Concurrency,
		RPMLimit:                  a.RPMLimit,
		ProbeMinInputTokens:       a.ProbeMinInputTokens,
...
func redactAccountManagedExtra(extra map[string]any) map[string]any {
	...
	for key, value := range extra {
		switch {
		case key == "upstream_rate_multiplier",
			key == "upstream_source_rate_multiplier",
			key == "upstream_recharge_rate",
			key == "upstream_effective_cost_multiplier",
...
func AccountListItemFromAccount(a *Account) *AccountListItem {
	...
	return &AccountListItem{
		...
		UpstreamKeyName:           a.UpstreamKeyName,
		UpstreamKeyMasked:         a.UpstreamKeyMasked,
		UpstreamImagePricing:      a.UpstreamImagePricing,
		UpstreamVideoPricing:      a.UpstreamVideoPricing,
		UpstreamLongContext:       a.UpstreamLongContext,
		UpstreamModelSync:         a.UpstreamModelSync,
		UpstreamSchedulingEnabled: a.UpstreamSchedulingEnabled,
...
func ProxyFromService(p *service.Proxy) *Proxy {
	...
	return &Proxy{
		...
		ProxyID:              p.ProxyID,
		ProxyIPGroupID:       p.ProxyIPGroupID,
		Protocol:             p.Protocol,
		Host:                 p.Host,
		Port:                 p.Port,
		Username:             p.Username,
		...
		AvailableMemberCount: p.AvailableMemberCount,
		PerIPConcurrency:     p.PerIPConcurrency,
	}
}
