...
const (
	...
	maxGrokWebSearchQueryBytes  = 32 * 1024
)
...
func (h *GatewayHandler) WebSearch(c *gin.Context) {
	...
	queryInput := strings.TrimSpace(req.Query)
	if queryInput == "" {
		queryInput = strings.TrimSpace(req.Input)
	}
	query, queryErr := normalizeGrokWebSearchQuery(queryInput)
	if errors.Is(queryErr, errGrokWebSearchQueryRequired) {
		c.JSON(http.StatusBadRequest, gin.H{"error": gin.H{
			"type":    "invalid_request_error",
	...
	if errors.Is(queryErr, errGrokWebSearchQueryTooLarge) {
...
var (
	errGrokWebSearchQueryRequired = errors.New("grok web search query is required")
	errGrokWebSearchQueryTooLarge = errors.New("grok web search query is too large")
)

func normalizeGrokWebSearchQuery(query string) (string, error) {
	query = strings.TrimSpace(query)
	if query == "" {
		return "", errGrokWebSearchQueryRequired
	}
	if len(query) > maxGrokWebSearchQueryBytes {
		return "", errGrokWebSearchQueryTooLarge
	}
	return query, nil
}
