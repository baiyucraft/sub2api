...
type upsertUpstreamKeyModelRouteRequest struct {
...
func (h *UpstreamConfigHandler) ListModelRoutes(c *gin.Context) {
	configID, ok := parseUpstreamIDParam(c, "id")
	if !ok {
		return
	}
	if _, err := h.service.GetByID(c.Request.Context(), configID); err != nil {
		response.ErrorFrom(c, err)
		return
	}
	routes, err := h.service.ListModelRoutes(c.Request.Context(), configID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *UpstreamConfigHandler) ListKeyModelRoutes(c *gin.Context) {
	configID, keyID, ok := h.modelRouteKeyParams(c)
	if !ok {
		return
	}
	routes, err := h.service.ListKeyModelRoutes(c.Request.Context(), keyID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *UpstreamConfigHandler) CreateKeyModelRoute(c *gin.Context) {
	_, keyID, ok := h.modelRouteKeyParams(c)
	if !ok {
		return
	}
	var req upsertUpstreamKeyModelRouteRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}
	...
	route := &service.UpstreamKeyModelRoute{
		UpstreamKeyID: keyID, PublicModel: req.PublicModel, UpstreamModel: req.UpstreamModel,
		TargetPlatform: req.TargetPlatform, APIProtocol: req.APIProtocol,
		Enabled: enabled, Priority: req.Priority,
	}
	if err := h.service.SaveKeyModelRoute(c.Request.Context(), route); err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *UpstreamConfigHandler) UpdateKeyModelRoute(c *gin.Context) {
	_, keyID, ok := h.modelRouteKeyParams(c)
	if !ok {
		return
	}
	routeID, ok := parseUpstreamIDParam(c, "routeID")
	if !ok {
		return
	}
	existingRoutes, err := h.service.ListKeyModelRoutes(c.Request.Context(), keyID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	if !containsUpstreamKeyModelRoute(existingRoutes, routeID) {
		response.NotFound(c, "Upstream key model route not found")
		return
	}
	var req upsertUpstreamKeyModelRouteRequest
	if err := c.ShouldBindJSON(&req); err != nil {
	...
	route := &service.UpstreamKeyModelRoute{
		ID: routeID, UpstreamKeyID: keyID, PublicModel: req.PublicModel, UpstreamModel: req.UpstreamModel,
		TargetPlatform: req.TargetPlatform, APIProtocol: req.APIProtocol,
		Enabled: enabled, Priority: req.Priority,
	}
	if err := h.service.SaveKeyModelRoute(c.Request.Context(), route); err != nil {
...
func (h *UpstreamConfigHandler) DeleteKeyModelRoute(c *gin.Context) {
	_, keyID, ok := h.modelRouteKeyParams(c)
	if !ok {
		return
	}
	...
	if err := h.service.DeleteKeyModelRoute(c.Request.Context(), keyID, routeID); err != nil {
		response.ErrorFrom(c, err)
		return
	}
	response.Success(c, gin.H{"message": "upstream key model route deleted"})
}

func (h *UpstreamConfigHandler) RestoreKeyModelRouteAuto(c *gin.Context) {
	_, keyID, ok := h.modelRouteKeyParams(c)
	if !ok {
		return
	}
	...
	route, err := h.service.RestoreKeyModelRouteAuto(c.Request.Context(), keyID, routeID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *UpstreamConfigHandler) modelRouteKeyParams(c *gin.Context) (int64, int64, bool) {
	configID, ok := parseUpstreamIDParam(c, "id")
	if !ok {
		return 0, 0, false
	}
	...
	key, err := h.service.GetKeyByID(c.Request.Context(), keyID)
	if err != nil {
		response.ErrorFrom(c, err)
		return 0, 0, false
	}
	if key == nil || key.UpstreamConfigID != configID {
		response.NotFound(c, "Upstream key not found")
		return 0, 0, false
	}
	return configID, keyID, true
}

func containsUpstreamKeyModelRoute(routes []service.UpstreamKeyModelRoute, routeID int64) bool {
	for i := range routes {
