...
func newSnapshotCache(ttl time.Duration) *snapshotCache {
	return newBoundedSnapshotCache(ttl, 0)
}
...
func (c *snapshotCache) Set(key string, payload any) snapshotCacheEntry {
	...
	if c.maxEntries > 0 {
		if _, exists := c.items[key]; !exists && len(c.items) >= c.maxEntries {
			...
			for itemKey, item := range c.items {
				if oldestKey == "" || item.ExpiresAt.Before(oldestExpiry) || (item.ExpiresAt.Equal(oldestExpiry) && itemKey < oldestKey) {
					oldestKey = itemKey
...
// Clear drops all in-process snapshots after a cross-cutting ledger mutation.
