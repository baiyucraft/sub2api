...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			if !selection.Acquired {
				...
				canWait, err := h.concurrencyHelper.IncrementAccountWaitCountForAccount(c.Request.Context(), account, selection.WaitPlan.MaxWaiting)
				if err != nil {
			...
			if account.EffectivePlatform() == service.PlatformAntigravity {
			...
			if err != nil {
				...
				if errors.As(err, &failoverErr) {
					...
					action := fs.HandleFailoverError(c.Request.Context(), h.gatewayService, account.ID, account.EffectivePlatform(), account.GetPoolModeRetryCount(), failoverErr)
					switch action {
					case FailoverContinue:
						continue
				...
				forwardFailedFields := []zap.Field{
					...
					zap.String("account_platform", account.EffectivePlatform()),
	...
	defer func() {
		h.releaseUnservedAccountSessions(sessionSlotAccounts, sessionKey, upstreamServedSession)
	}()
	...
	for {
		...
		for {
			...
			// Selection has already registered the session, including wait plans.
			// Track it before interception, queue admission, or profit veto can return.
			...
			// 记录 Forward 前已写入字节数，Forward 后若增加则说明 SSE 内容已发，禁止 failover
			writerSizeBeforeForward := c.Writer.Size()
			if account.EffectivePlatform() == service.PlatformAntigravity && account.Type != service.AccountTypeAPIKey {
			...
			if err != nil {
				...
				if errors.As(err, &failoverErr) {
					// 流式内容已写入客户端，无法撤销，禁止 failover 以防止流拼接腐化
					if c.Writer.Size() != writerSizeBeforeForward {
						h.handleFailoverExhausted(c, failoverErr, account.EffectivePlatform(), true)
						return
					}
					action := fs.HandleFailoverError(c.Request.Context(), h.gatewayService, account.ID, account.EffectivePlatform(), account.GetPoolModeRetryCount(), failoverErr)
					switch action {
					case FailoverContinue:
						...
						continue
					case FailoverExhausted:
						h.handleFailoverExhausted(c, fs.LastFailoverErr, account.EffectivePlatform(), streamStarted)
						return
					case FailoverCanceled:
						failoverClientGone(c)
						return
...
func (h *GatewayHandler) releaseUnservedAccountSessions(accounts map[int64]*service.Account, sessionKey string, upstreamServed bool) {
	if upstreamServed {
		return
	}
	// Release must survive cancellation of the client request.
...
// modelListingSource 决定模型列表过滤的候选来源：账户映射（availableModels）
// 与平台默认列表（fallbackModels）。账户映射为空时回退默认列表；Anthropic
// 平台额外并入默认列表和 Antigravity 兼容模型。其余平台以账户映射为准。
