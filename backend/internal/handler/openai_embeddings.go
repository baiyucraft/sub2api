...
func (h *OpenAIGatewayHandler) Embeddings(c *gin.Context) {
	...
	c.Request = c.Request.WithContext(service.WithGatewayInputTokenEstimate(
		c.Request.Context(), service.EstimateGatewayInputTokens(body, "embeddings"),
	))
	...
	for {
		...
		if slotResult == openAISlotAcquireCapacityFull {
			if h.handleOpenAICapacityFull(c, forwardModel, account, failedAccountIDs, false, reqLog) {
				continue
			}
			return
		}
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				if !failoverErr.PluginAdmissionRejected {
					h.gatewayService.ReportOpenAIAccountScheduleResultForGroup(apiKey.GroupID, account, openAIAccountScheduleModel(c, account, reqModel, false, result), false, nil, err)
				}
				if failoverClientGone(c) {
