...
type openAIQuotaUpstreamCall struct {
	accessToken        string
	chatGPTAccountID   string
	proxyURL           string
	fedRAMP            bool
	releaseProxyEgress func()
}
...
func (s *OpenAIQuotaService) QueryUsage(ctx context.Context, accountID int64) (*OpenAIQuotaUsage, error) {
	call, err := s.prepareUpstreamCall(ctx, accountID)
	if err != nil {
		return nil, err
	}
	defer call.releaseProxyEgress()

	client, err := s.privacyClientFactory(call.proxyURL)
	if err != nil {
	...
	for recovered := false; ; {
		quotaHeaders, expectedTaskID, headerErr := s.buildCodexQuotaHeaders(callCtx, accountID, call.accessToken, call.chatGPTAccountID, call.fedRAMP)
		if headerErr != nil {
	...
	details := s.queryResetCreditDetails(callCtx, client, call.accessToken, call.chatGPTAccountID, call.fedRAMP, accountID)
	if details != nil {
...
func (s *OpenAIQuotaService) resetCredit(ctx context.Context, accountID int64, creditID, redeemRequestID string, targeted bool) (*OpenAIQuotaResetResult, error) {
	...
	for recovered := false; ; {
		headers, expectedTaskID, headerErr := s.buildCodexQuotaHeaders(callCtx, accountID, call.accessToken, call.chatGPTAccountID, call.fedRAMP)
		if headerErr != nil {
			return nil, infraerrors.Newf(http.StatusBadGateway, "OPENAI_QUOTA_AUTH_FAILED", "failed to build upstream authentication: %v", headerErr)
		}
...
func (s *OpenAIQuotaService) prepareUpstreamCall(ctx context.Context, accountID int64) (*openAIQuotaUpstreamCall, error) {
	...
	resolver, _ := s.agentIdentityWS.(OpenAIManagementEgressResolver)
	resolvedAccount, releaseProxyEgress, resolveErr := acquireOpenAIManagementEgress(ctx, resolver, account)
	if resolveErr != nil {
		return nil, openAIManagementEgressError(resolveErr)
	}
	...
	keepLease = true
	return &openAIQuotaUpstreamCall{
		accessToken:        accessToken,
		chatGPTAccountID:   chatGPTAccountID,
		proxyURL:           proxyURL,
		fedRAMP:            fedRAMP,
		releaseProxyEgress: releaseProxyEgress,
	}, nil
}
