...
func (s *OpenAIQuotaService) referralCall(ctx context.Context, id int64, program string) (OpenAIReferralCall, func(), error) {
	if s.referralClient == nil {
		return OpenAIReferralCall{}, nil, infraerrors.New(http.StatusServiceUnavailable, "OPENAI_REFERRAL_NOT_CONFIGURED", "referral service is unavailable")
	}
	upstream, err := s.prepareUpstreamCall(ctx, id)
	if err != nil {
		return OpenAIReferralCall{}, nil, err
	}
	headers, _, err := s.buildCodexQuotaHeaders(ctx, id, upstream.accessToken, upstream.chatGPTAccountID, upstream.fedRAMP)
	if err != nil {
		upstream.releaseProxyEgress()
		return OpenAIReferralCall{}, nil, infraerrors.New(http.StatusBadGateway, "OPENAI_REFERRAL_AUTH_ERROR", "failed to authenticate referral request")
	}
	return OpenAIReferralCall{ProxyURL: upstream.proxyURL, Headers: headers, ProgramID: program}, upstream.releaseProxyEgress, nil
}
...
func (s *OpenAIQuotaService) QueryReferralEligibility(ctx context.Context, id int64) (*OpenAIReferralEligibility, error) {
	...
	call, release, err := s.referralCall(callCtx, id, program)
	if err != nil {
		return nil, err
	}
	defer release()
...
func (s *OpenAIQuotaService) SendReferralInvite(ctx context.Context, id int64, input OpenAIReferralSendRequest) (*OpenAIReferralSendResult, error) {
	...
	callCtx, cancel := context.WithTimeout(ctx, openaiQuotaUpstreamTimeout)
	defer cancel()
	call, release, err := s.referralCall(callCtx, id, eligibility.ProgramID)
	if err != nil {
		return nil, err
	}
	defer release()
