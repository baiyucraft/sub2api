...
func ProvideUpstreamHealthProbeRunner(service *UpstreamConfigService) *UpstreamHealthProbeRunner {
	...
	startReleaseActivatedTask(func() { runner.Start(context.Background()) })
	return runner
}
...
func ProvideSub2APIUpstreamRateSyncService(accountRepo AccountRepository, upstreamConfigRepo UpstreamConfigRepository, proxyRepo ProxyRepository, upstreamConfigSvc *UpstreamConfigService, cfg *config.Config) *Sub2APIUpstreamRateSyncService {
	...
	if cfg != nil {
		svc.SetAutoSyncEnabled(cfg.UpstreamSync.AutoEnabled)
	}
	startReleaseActivatedTask(svc.Start)
	return svc
}
...
func ProvideOpsMetricsCollector(
	...
	startReleaseActivatedTask(collector.Start)
	return collector
}
...
func ProvideSettingService(settingRepo SettingRepository, groupRepo GroupRepository, proxyRepo ProxyRepository, cfg *config.Config) *SettingService {
	...
	// Session switching is optional; warm its snapshot once so gateway requests
	// never need to synchronously query the settings table.
	svc.WarmSessionSwitchSettings(context.Background())
...
func ProvideBalanceNotifyService(emailService *EmailService, settingRepo SettingRepository, accountRepo AccountRepository, userRepo UserRepository, notificationEmailService *NotificationEmailService) *BalanceNotifyService {
	svc := NewBalanceNotifyService(emailService, settingRepo, accountRepo, userRepo)
	svc.SetNotificationEmailService(notificationEmailService)
	return svc
}

// ProvidePaymentService creates PaymentService and attaches notification email delivery.
func ProvidePaymentService(entClient *dbent.Client, registry *payment.Registry, loadBalancer payment.LoadBalancer, redeemService *RedeemService, subscriptionSvc *SubscriptionService, configService *PaymentConfigService, userRepo UserRepository, groupRepo GroupRepository, affiliateService *AffiliateService, notificationEmailService *NotificationEmailService, dailyActivityService *DailyActivityService) *PaymentService {
	svc := NewPaymentService(entClient, registry, loadBalancer, redeemService, subscriptionSvc, configService, userRepo, groupRepo, affiliateService)
	svc.SetNotificationEmailService(notificationEmailService)
	svc.SetDailyActivityService(dailyActivityService)
	return svc
}
...
func ProvideChannelMonitorService(
	...
	svc.SetManagedMonitorDependencies(apiKeyService, settingService)
...
func ProvideChannelMonitorV2Aggregator(repo ChannelMonitorV2Repository, db *sql.DB, settingService *SettingService) *ChannelMonitorV2Aggregator {
	...
	startReleaseActivatedTask(aggregator.Start)
	return aggregator
}
