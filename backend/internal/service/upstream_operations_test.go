...
func TestNormalizeUpstreamActualRate(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 0.06555, actual)
	...
	actual, err = NormalizeUpstreamActualRate(0.045, 1)
	require.NoError(t, err)
	require.Equal(t, 0.045, actual)
	require.Equal(t, 5, Sub2APIUpstreamPriority(actual))
