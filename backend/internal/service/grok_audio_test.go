...
func TestGrokRealtimeMaxSessionDurationUsesLiveLimit(t *testing.T) {
	require.Equal(t, time.Hour, (&OpenAIGatewayService{}).GrokRealtimeMaxSessionDuration())
	require.Equal(t, 90*time.Second, (&OpenAIGatewayService{cfg: &config.Config{
		Gateway: config.GatewayConfig{Live: config.GatewayLiveConfig{MaxSessionDurationSeconds: 90}},
	}}).GrokRealtimeMaxSessionDuration())
}
