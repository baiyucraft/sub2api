...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrderWithBudget(
	...
	fullTargets := make(map[string]struct{})
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		if candidate.loadKnown && target.Limit > 0 &&
			candidate.loadInfo.CurrentConcurrency >= target.Limit {
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAIAccountSlot(
	...
	if s.service.concurrencyService != nil && target.Limit > 0 && !budget.recordAcquire(account.ID) {
		return nil, false, nil
	}
...
// selectAccountWithSchedulerOnce wraps selectAccountWithSchedulerCore with a
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	return s.selectAccountWithSchedulerCore(withOpenAIProxyStreamQuarantineBypass(ctx), groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, previousResponseExcludedIDs, separatePreviousExclusions, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
}
...
func (s *OpenAIGatewayService) selectLegacyAccountByPreviousResponse(
	...
	if openAICapacityTargetKeyExcluded(openAICapacityExcludedTargets(ctx), account) {
		if selection.ReleaseFunc != nil {
			selection.ReleaseFunc()
		}
		return nil, false, nil
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerCore(
	...
	return scheduler.Select(ctx, OpenAIAccountScheduleRequest{
		...
		PreserveStickyBinding:       openAITTFTGuardExcludedAccount(ctx, stickyAccountID) || preserveGuardianParentBinding,
		RequirePrivacySet:           s.openAIGroupRequiresPrivacySet(ctx, groupID),
...
func (s *OpenAIGatewayService) reportOpenAIAccountScheduleResult(groupID *int64, accountRef any, model string, success bool, firstTokenMs *int, observedErr ...error) bool {
	...
	if s != nil && s.rateLimitService != nil {
		...
		} else if account != nil && len(observedErr) > 0 && observedErr[0] != nil {
...
func openAISchedulingRate(account *Account, now time.Time, oauthSchedulingRateMultiplier *float64) (float64, bool) {
	...
	if account.IsUpstreamBound() || account.IsSub2APIUpstream() {
