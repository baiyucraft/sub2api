...
func TestAccountTestService_OpenAIImageOAuthHandlesOutputItemDoneFallback(t *testing.T) {
	...
	metadata := upstream.lastReq.Context().Value(pluginRequestMetadataKey{}).(pluginRequestMetadata)
	require.Equal(t, extractOpenAIOutboundModel(upstream.lastBody), metadata.Model)
	require.Equal(t, openAIImagesResponsesMainModelValue(), metadata.Model)
