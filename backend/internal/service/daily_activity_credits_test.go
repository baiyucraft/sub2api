...
func TestDailyActivitySyncCreditsUsesLifetimeIssuedCountAndStableIndexes(t *testing.T) {
	...
	mock.ExpectQuery("(?s)SUM\\(GREATEST\\(actual_cost, 0\\)\\).*AT TIME ZONE 'Asia/Shanghai'").
		...
		WillReturnRows(sqlmock.NewRows([]string{"credits"}).AddRow(int64(1)))
	mock.ExpectQuery("SELECT aff_rebate_rate_percent IS NOT NULL FROM user_affiliates WHERE user_id=\\$1 FOR UPDATE").
		WithArgs(int64(7)).
		WillReturnRows(sqlmock.NewRows([]string{"exclusive"}).AddRow(false))
...
func TestDailyActivitySyncCreditsSkipsNewInviteCreditsForExclusiveRebateUser(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
	...
	mock.ExpectQuery("SELECT aff_rebate_rate_percent IS NOT NULL FROM user_affiliates WHERE user_id=\\$1 FOR UPDATE").
		WithArgs(int64(7)).
		WillReturnRows(sqlmock.NewRows([]string{"exclusive"}).AddRow(true))
	mock.ExpectQuery("SELECT id FROM users WHERE id=\\$1 FOR UPDATE").
		WithArgs(int64(7)).
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(int64(7)))
