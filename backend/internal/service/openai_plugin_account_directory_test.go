...
func TestListPluginAccounts_ScopeAndSchedulable(t *testing.T) {
	...
	parentID := int64(1)
	openai := []Account{
		// schedulable active oauth. Credentials and Extra are host-private.
	...
	// Metadata is an explicit scheduling snapshot and NEVER includes credentials,
	// Extra, names, proxy fields, or upstream error text.
	...
	assert.NotContains(t, meta, "ek-value", "Extra must remain host-private")
	assert.NotContains(t, meta, "openai_compact_mode", "Extra must remain host-private")
	assert.Equal(t, "primary", got[1].Name, "readable name is a typed field")
	assert.NotContains(t, string(got[2].MetadataJSON), "429 from upstream", "pause reason must remain host-private")
}
...
func TestAccountReadableSnapshot_DenylistTripwire(t *testing.T) {
	// The snapshot is now a strict four-field scheduling projection. Credentials,
	// Extra, proxy material, and relational graphs never cross the plugin boundary.
	stripped := map[string]struct{}{
		"Credentials": {}, "Extra": {}, "Proxy": {}, "Groups": {}, "AccountGroups": {},
		"ProxyIPGroupID":   {},
		"UpstreamConfigID": {}, "UpstreamKeyID": {}, "UpstreamSiteURL": {}, "UpstreamConfigName": {},
		"UpstreamKeyName": {}, "UpstreamKeyMasked": {}, "UpstreamImagePricing": {}, "UpstreamVideoPricing": {},
		"UpstreamLongContext": {}, "UpstreamSchedulingEnabled": {}, "UpstreamConcurrencyLimit": {},
		"UpstreamConcurrencySource": {}, "UpstreamConcurrencyUsesDefault": {}, "UpstreamConcurrencyUnlimited": {},
		"UpstreamConcurrencyOverride": {}, "UpstreamStalePauseKeyID": {}, "UpstreamStalePausedAt": {},
		"UpstreamLifecycleOwner": {}, "UpstreamArchiveReason": {}, "ProbeMinInputTokens": {},
		"UpstreamSourceRateMultiplier": {}, "ProxyIPGroup": {},
		"RPMLimit": {},
	}
	// Fields represented as typed PluginAccountInfo or as the four explicit
	// scheduling timestamps are not serialized wholesale, but remain classified.
	...
	// The raw Credentials blob, Extra, and proxy material must never serialize.
	acct := &Account{
	...
	assert.NotContains(t, string(snap), "extra-released", "Extra must remain host-private")
	assert.NotContains(t, string(snap), "pw-released", "proxy material must remain host-private")
