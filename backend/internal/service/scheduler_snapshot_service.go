...
func (s *SchedulerSnapshotService) runInitialRebuild() {
	...
	_ = s.coalesceFullRebuild(func() error {
		...
		defer cancel()
		if err := s.rebuildInitialSnapshot(ctx); err != nil {
...
func (s *SchedulerSnapshotService) rebuildFullSnapshot(ctx context.Context, reason string) error {
	return s.rebuildFullSnapshotWithLegacyRegistry(ctx, reason, false)
}

func (s *SchedulerSnapshotService) rebuildInitialSnapshot(ctx context.Context) error {
	return s.rebuildFullSnapshotWithLegacyRegistry(ctx, "startup", true)
}

func (s *SchedulerSnapshotService) rebuildFullSnapshotWithLegacyRegistry(ctx context.Context, reason string, allowLegacy bool) error {
	if s.cache == nil {
		return ErrSchedulerCacheNotReady
	}
	...
	if allowLegacy && len(registered) == 0 {
		if legacyCache, ok := s.cache.(legacySchedulerBucketLister); ok {
			registered, err = legacyCache.ListLegacyBuckets(ctx)
			if err != nil {
				return err
			}
		}
