...
// ApplyAnthropicOpenAIReasoningEffortPolicy enforces an OpenAI group's policy
// before the Anthropic Messages bridge chooses its Responses or raw-chat path.
// The bridge defaults omitted output_config.effort to medium and maps max to
// xhigh, so both the explicit field and model suffix must be considered.
func ApplyAnthropicOpenAIReasoningEffortPolicy(body []byte, maxEffort string, mappings []ReasoningEffortMapping) ([]byte, bool) {
	maxRank, hasMax := reasoningEffortRank(maxEffort)
	if len(body) == 0 || (!hasMax && len(mappings) == 0) {
		return body, false
	}
	...
	result := body
	model := strings.TrimSpace(gjson.GetBytes(result, "model").String())
	explicit := strings.TrimSpace(gjson.GetBytes(result, "output_config.effort").String())
	effective := explicit
	if effective == "" {
		if _, derived, ok := splitOpenAICompatReasoningModel(model); ok {
			effective = derived
		} else {
			// AnthropicToResponses injects medium when omitted.
			effective = "medium"
		}
	}
	if effective == "max" {
	...
	effective, _ = mapReasoningEffort(effective, mappings, model)
	...
	if explicit != "" && strings.EqualFold(strings.TrimSpace(explicit), effective) {
		return result, false
	}
	if explicit == "" && effective == "medium" {
		return result, false
	}
	updated, err := sjson.SetBytes(result, "output_config.effort", effective)
	if err != nil {
		return result, false
	}
	return updated, true
