...
func TestCodexFingerprintCreateUsesIndependentAccountSeeds(t *testing.T) {
	raw := map[string]any{
		codexFingerprintModeExtraKey:              "session",
		codexImportReplicaFingerprintSeedExtraKey: existingCodexImportReplicaFingerprintSeed,
	}
	accountAExtra := prepareCodexFingerprintExtraForCreate(PlatformOpenAI, AccountTypeOAuth, raw)
	accountBExtra := prepareCodexFingerprintExtraForCreate(PlatformOpenAI, AccountTypeOAuth, raw)
	...
	seedA, ok := codexFingerprintSeed(accountAExtra)
	require.True(t, ok)
	seedB, ok := codexFingerprintSeed(accountBExtra)
	require.True(t, ok)
	require.NotEqual(t, seedA, seedB)

	idsA := resolveCodexFingerprintIDs(newTestOAuthAccount(5101, accountAExtra), "same-client", codexFingerprintSession)
	idsB := resolveCodexFingerprintIDs(newTestOAuthAccount(5102, accountBExtra), "same-client", codexFingerprintSession)
...
func TestResolveCodexFingerprintIDsIgnoresLegacyReplicaSeed(t *testing.T) {
	legacySeed := "44444444-4444-4444-8444-444444444444"
	account := newTestOAuthAccount(5131, map[string]any{
		codexFingerprintModeExtraKey:              "session",
		codexFingerprintSeedExtraKey:              testCodexFingerprintSeedB,
		codexImportReplicaFingerprintSeedExtraKey: legacySeed,
	})
	...
	require.NotEqual(t, resolveConvergedSessionID(legacySeed), ids.sessionID)
}
