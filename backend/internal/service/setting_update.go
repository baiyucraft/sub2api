...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	// Partial service-level updates historically pass a zero-value
	// SystemSettings struct. Treat a zero-value activity config as omitted so
	// unrelated settings updates preserve the stored activity rules. The
	// admin handler supplies the previous config for partial requests; any
	// explicit non-zero config still goes through full validation.
	if settings.DailyActivityConfig != (DailyActivityConfig{}) {
	...
	updates[SettingKeyUpstreamBalanceNotifyEnabled] = strconv.FormatBool(settings.UpstreamBalanceNotifyEnabled)
