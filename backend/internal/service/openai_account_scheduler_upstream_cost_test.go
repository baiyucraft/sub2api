...
func TestOpenAISchedulingRateUsesUpstreamActualRateWithoutProbeOrRecharge(t *testing.T) {
	configID, keyID := int64(7), int64(8)
	actualRate := 0.8
	account := upstreamCostTestAccount(9, UpstreamBillingProbeStatusOK, 8, time.Now(), 30*time.Minute)
	account.UpstreamConfigID = &configID
	account.UpstreamKeyID = &keyID
	account.RateMultiplier = &actualRate

	rate, ok := openAISchedulingRate(account, time.Now(), floatPtr(1))
	require.True(t, ok)
