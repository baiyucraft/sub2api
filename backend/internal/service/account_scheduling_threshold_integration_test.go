...
func TestOpenAIGatewayService_ListSchedulableAccounts_DoesNotApplyOAuthThresholdToUpstreamBoundAccount(t *testing.T) {
	accountSchedulingThresholdsSF.Forget(SettingKeyAccountSchedulingThresholds)
	accountSchedulingThresholdsCache.Store(&cachedAccountSchedulingThresholds{})

	settingsRepo := newMockSettingRepo()
	settingsRepo.data[SettingKeyAccountSchedulingThresholds] = `{"openai":85}`
	upstreamConfigID := int64(71)
	accountRepo := &thresholdSelectionAccountRepoStub{
		accounts: []Account{{
			ID:               4201,
			Platform:         PlatformOpenAI,
	...
	require.NoError(t, err)
	require.Len(t, accounts, 1)
	require.Equal(t, int64(4201), accounts[0].ID)
	require.Equal(t, 0, accountRepo.tempCalls)
}
