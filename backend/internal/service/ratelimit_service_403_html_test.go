...
type upstreamHealthEvidenceRecorderStub struct {
	failures int
}

func (s *upstreamHealthEvidenceRecorderStub) RecordUpstreamTrafficSuccess(context.Context, *Account, int) {
}
func (s *upstreamHealthEvidenceRecorderStub) RecordUpstreamTrafficFailure(context.Context, *Account, int) {
	s.failures++
}
...
func TestHandleUpstreamError_OpenAIHTML403DoesNotPenalizeAccount(t *testing.T) {
	recorder := &upstreamHealthEvidenceRecorderStub{}
	SetGlobalUpstreamHealthEvidenceRecorder(recorder)
	t.Cleanup(func() { SetGlobalUpstreamHealthEvidenceRecorder(nil) })
	cases := []struct {
		name string
		body string
	}{
	...
	require.Zero(t, recorder.failures, "HTML 403 must not degrade or suspend the upstream Key")
}
...
func TestHandleUpstreamError_OpenAIStructured403StillPenalizes(t *testing.T) {
	...
	require.Equal(t, 3, recorder.failures, "structured and plain-text 403 responses remain Key health evidence")
}
