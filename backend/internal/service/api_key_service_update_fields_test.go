...
func TestAPIKeyUpdate_ManagedMonitorKeyIsNotEditable(t *testing.T) {
	name := "renamed"
	svc, repo := newUpdateFieldsAPIKeyService(&APIKey{
		ID:      1,
		UserID:  7,
		Key:     "sk-managed",
		Name:    "managed",
		Purpose: APIKeyPurposeManagedMonitor,
		Status:  StatusActive,
	})
...
func TestAPIKeyDelete_ManagedMonitorKeyIsNotDeletable(t *testing.T) {
	svc, repo := newUpdateFieldsAPIKeyService(&APIKey{
		ID:      1,
		UserID:  7,
		Key:     "sk-managed",
