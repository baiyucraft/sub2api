...
func TestHandleUpstreamError_KimiCapacity403DoesNotSuspendUpstreamKey(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "quota_exhausted", body: kimiWeeklyQuotaExhaustedBody},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, runKimi403(service, account, tc.body))
		})
	}

	require.Zero(t, recorder.failures, "recoverable Kimi capacity 403s must not suspend the shared upstream Key")
}
