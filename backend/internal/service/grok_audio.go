...
// GrokRealtimeMaxSessionDuration applies the same bounded session lifetime as
// OpenAI Live so a voice relay cannot hold an account slot indefinitely.
func (s *OpenAIGatewayService) GrokRealtimeMaxSessionDuration() time.Duration {
	return s.liveMaxSessionDuration()
}
