...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	dailyActivityService := service.NewDailyActivityService(db, settingService)
	v := handler.ProvideDailyActivityServices(dailyActivityService, billingCacheService)
	userHandler := handler.NewUserHandler(userService, authService, emailService, emailCache, affiliateService, serviceUserPlatformQuotaRepository, v...)
	...
	groupTTFTGuardPolicyRepository := repository.NewGroupTTFTGuardPolicyRepository(db)
	groupTTFTGuardPolicyInvalidationBus := repository.NewGroupTTFTGuardPolicyInvalidationBus(redisClient)
	groupTTFTGuardPolicyService := service.NewGroupTTFTGuardPolicyService(groupTTFTGuardPolicyRepository, settingService, groupTTFTGuardPolicyInvalidationBus)
	schedulerSnapshotService := service.ProvideSchedulerSnapshotService(schedulerCache, schedulerOutboxRepository, accountRepository, groupRepository, configConfig, groupTTFTGuardPolicyService)
	...
	balanceNotifyService := service.ProvideBalanceNotifyService(emailService, settingRepository, accountRepository, userRepository, notificationEmailService)
	...
	proxyIPGroupRepository := repository.NewProxyIPGroupRepository(client)
	...
	openAIGatewayService := service.ProvideOpenAIGatewayService(accountRepository, proxyRepository, proxyIPGroupRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, serviceUserPlatformQuotaRepository, groupTTFTGuardPolicyService)
	...
	channelMonitorService := service.ProvideChannelMonitorService(channelMonitorRepository, secretEncryptor, apiKeyService, settingService)
	channelMonitorUserHandler := handler.NewChannelMonitorUserHandler(channelMonitorService, settingService, apiKeyService)
	...
	extraCostRepository := repository.NewExtraCostRepository(db)
	extraCostService := handler.ProvideExtraCostService(extraCostRepository, dashboardService)
	v2 := handler.ProvideExtraCostServices(extraCostService)
	dashboardHandler := admin.NewDashboardHandler(dashboardService, dashboardAggregationService, v2...)
	...
	upstreamConfigRepository := repository.ProvideUpstreamConfigRepository(client, schedulerCache)
	...
	pluginStateStore := repository.NewPluginRuntimeStateStore(db, secretEncryptor)
	pluginManager := service.ProvidePluginManager(pluginRepository, secretEncryptor, configConfig, pluginHostInfo, pluginKVStore, openAIGatewayService, pluginStateStore)
	...
	upstreamConfigService := service.ProvideUpstreamConfigService(upstreamConfigRepository, upstreamAuthSessionRepository, secretEncryptor, proxyRepository, accountRepository, accountTestService, settingService, balanceNotifyService)
	...
	adminService := service.NewAdminService(configConfig, userRepository, adminGroupRepository, adminAccountRepository, upstreamConfigRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, openAIGatewayService, sub2APIUpstreamRateSyncService, affiliateService, dailyActivityService, compositeModelRouteRepository, compositeRouteResolver, channelService, proxyIPGroupRepository)
	...
	groupHandler := handler.ProvideAdminGroupHandler(adminService, dashboardService, groupCapacityService, configConfig, groupTTFTGuardPolicyService)
	...
	proxyIPGroupAdminService := service.NewProxyIPGroupAdminService(proxyIPGroupRepository, proxyRepository)
	proxyIPGroupHandler := admin.NewProxyIPGroupHandler(proxyIPGroupAdminService)
	...
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, dailyActivityService)
	...
	adminUsageHandler := admin.NewUsageHandler(usageService, apiKeyService, adminService, usageCleanupService, v2...)
	...
	upstreamConfigHandler := admin.NewUpstreamConfigHandler(upstreamConfigService)
	...
	extraCostHandler := admin.NewExtraCostHandler(extraCostService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	openCodeGoUsageService := service.ProvideOpenCodeGoUsageService(accountRepository, httpUpstream, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, proxyIPGroupHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, upstreamConfigHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, extraCostHandler, upstreamBillingProbeService, ollamaCloudUsageService, proxyIPGroupAdminService, openCodeGoUsageService)
	...
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, pluginManager, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, settingService, configConfig, coordinator)
	...
	prepareShutdownFunc := providePrepareShutdown(channelMonitorRunner)
	...
	upstreamHealthProbeRunner := service.ProvideUpstreamHealthProbeRunner(upstreamConfigService)
	v3 := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, schedulerSnapshotService, tokenRefreshService, accountExpiryService, sub2APIUpstreamRateSyncService, cnProviderBalanceCheckService, openAICodexVersionSyncService, claudeCodeVersionSyncService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, channelMonitorV2Aggregator, userPlatformQuotaUsageFlusher, upstreamBillingProbeService, upstreamHealthProbeRunner, ollamaCloudUsageService, openCodeGoUsageService, auditLogService, openAIQuotaAutoResetService, promptService, pluginManager, observerService)
