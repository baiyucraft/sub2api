...
func TestMigration235CreatesGroupUsageRollups(t *testing.T) {
	content, err := FS.ReadFile("235_group_usage_daily_rollups.sql")
...
func TestMigration235InvalidatesClosedBucketsWhenUsageLogsChange(t *testing.T) {
	content, err := FS.ReadFile("235_group_usage_daily_rollups.sql")
	require.NoError(t, err)
...
func TestMigration236TracksConfiguredTimezone(t *testing.T) {
	content, err := FS.ReadFile("236_group_usage_rollup_timezone.sql")
	require.NoError(t, err)
