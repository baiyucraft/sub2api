...
class ReleaseCoreTest(unittest.TestCase):
    ...
    def test_vm_old_image_compatibility_failures_are_classified_without_logs(self) -> None:
        ...
        self.assertIn('wget -S -O /dev/null -T 10 http://127.0.0.1:$server_port/api/v1/auth/me', validator)
        self.assertNotIn('wget -S -O /dev/null --timeout=10', validator)
        self.assertIn("grep -Eo 'HTTP/[0-9.]+[[:space:]]+[0-9]{3}'", validator)
        self.assertIn("awk '{print $2}' | tail -n1 || true", validator)
    ...
    def test_profile_195_requires_semantic_migration_evidence(self) -> None:
        ...
        self.assertIn("mark_stage runtime_fixture_profile_206", validator)
