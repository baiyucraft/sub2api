...
grep -Eiq "^x-sub2api-instance:[[:space:]]*$release_id\r?$" "$health_headers"
...
internal_large_code=$(curl -sS --max-time 30 -D "$tmp/large.internal.headers" -o "$tmp/large.internal.body" -w '%{http_code}' -H 'Content-Type: application/json' -H "x_release_probe: $request_marker" --data-binary @"$tmp/body.bin" "http://127.0.0.1:${candidate_port}/api/v1/auth/login" || true)
