...
if [[ $test_mode == true ]]; then
  ...
  if [[ -e $target_libexec_dir ]]; then
    ...
    [[ -d $target_libexec_dir && ! -L $target_libexec_dir && $(stat -c '%U:%G:%a' "$target_libexec_dir") == root:root:755 ]]
  fi
fi
...
install -d -m 755 "$target_libexec_dir"
...
selftest_release="195-000000000000-0-$selftest_suffix"
selftest_now=$(date -u +%Y-%m-%dT%H:%M:%SZ)
selftest_drill="dr-195-$(tr -d ':-' <<<"$selftest_now")"
...
cleanup() {
  ...
  rm -rf -- "$activation_dir" "/opt/sub2api-deploy/release-gates/$selftest_release" "/opt/sub2api-deploy/dr-evidence/$selftest_release"
}
trap cleanup EXIT
...
install -d -o root -g root -m 700 "$selftest_gate_dir"
...
install -d -o root -g root -m 700 "/opt/sub2api-deploy/dr-evidence/$selftest_release"
install -d -o root -g root -m 700 "$selftest_dr_dir"
