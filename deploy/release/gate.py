...
def verify_gate(bundle_dir: Path, public_key: Path, expected_profile: str, allow_expired: bool = False) -> dict[str, Any]:
    ...
    if expected_profile in {"194", "195", "197", "198", "199", "202", "206"} and evidence.get("prompt_audit_disabled") is not True:
        raise RuntimeError("gate lacks Prompt Audit disabled-state evidence")
    if expected_profile in {"195", "197", "198", "199", "202", "206"}:
    ...
    if expected_profile in {"198", "199", "202", "206"} and evidence.get("managed_monitor_key_names_verified") is not True:
        raise RuntimeError("gate lacks managed monitor key-name evidence")
    if expected_profile in {"199", "202", "206"}:
    ...
    if expected_profile in {"202", "206"}:
