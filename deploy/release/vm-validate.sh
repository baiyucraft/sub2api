...
if [[ $profile == 232 || $profile == 233 || $profile == 234 ]]; then
  if [[ $profile == 232 ]]; then
    ...
    expected_tail='216_channel_monitor_v2.sql|217_channel_monitor_mode.sql|218_channel_monitor_v2_ignored_error_categories.sql|219_channel_monitor_v2_seed_popular_models.sql|220_channel_monitor_v2_health_thresholds.sql|221_channel_monitor_v2_fixed_rollups.sql|222_channel_monitor_v2_rollup_permissions.sql|223_channel_monitor_v2_refresh_5m.sql|224_channel_monitor_v2_full_table_permissions.sql|225_channel_monitor_v2_default_ignore_and_cache.sql|226_channel_monitor_hide_throughput.sql|227_channel_monitor_v2_reset_factory_cache_thresholds.sql|228_channel_monitor_v2_privacy_defaults.sql|229_group_video_model_prices.sql|230_group_audio_voice_pricing.sql|231_group_search_price_per_1k.sql|232_clear_non_grok_video_generation_config.sql|233_upstream_account_key_unique.sql'
    [[ $(jq -er '.migrations[-18:] | join("|")' "$manifest") == "$expected_tail" ]]
  else
    [[ $version == 0.1.175-baiyu ]]
    [[ $(jq -er '.migrations | length' "$manifest") == 51 ]]
    expected_tail='216_channel_monitor_v2.sql|217_channel_monitor_mode.sql|218_channel_monitor_v2_ignored_error_categories.sql|219_channel_monitor_v2_seed_popular_models.sql|220_channel_monitor_v2_health_thresholds.sql|221_channel_monitor_v2_fixed_rollups.sql|222_channel_monitor_v2_rollup_permissions.sql|223_channel_monitor_v2_refresh_5m.sql|224_channel_monitor_v2_full_table_permissions.sql|225_channel_monitor_v2_default_ignore_and_cache.sql|226_channel_monitor_hide_throughput.sql|227_channel_monitor_v2_reset_factory_cache_thresholds.sql|228_channel_monitor_v2_privacy_defaults.sql|229_group_video_model_prices.sql|230_group_audio_voice_pricing.sql|231_group_search_price_per_1k.sql|232_clear_non_grok_video_generation_config.sql|233_upstream_account_key_unique.sql|234_upstream_health_observations.sql'
...
migration_234_postflight_verified=false
...
if [[ $profile == 233 || $profile == 234 ]]; then
  migration_233_status=$(profile_212_migration_status 233_upstream_account_key_unique.sql)
fi
if [[ $profile == 234 ]]; then
  migration_234_status=$(profile_212_migration_status 234_upstream_health_observations.sql)
fi
...
if [[ $profile == 233 || $profile == 234 ]]; then
  ...
  ASSERT_CONTEXT_FILE="$migration_233_context" ASSERT_DB_CONTAINER=sub2api-postgres ASSERT_DB_USER="$database_owner" ASSERT_DB_NAME="$probe_db" MIGRATION_STATUS="$migration_233_status" RELEASE_DIR="$state_dir" bash "$source_dir/deploy/maintenance/release/migration-233-assert.sh" preflight >/dev/null
  migration_233_preflight_verified=true
fi
if [[ $profile == 234 ]]; then
  migration_234_context="$state_dir/migration-234-context.sh"
  printf 'profile=%q\nstate_dir=%q\n' "$profile" "$state_dir" > "$migration_234_context"
  chmod 400 "$migration_234_context"
  ASSERT_CONTEXT_FILE="$migration_234_context" ASSERT_DB_CONTAINER=sub2api-postgres ASSERT_DB_USER="$database_owner" ASSERT_DB_NAME="$probe_db" MIGRATION_STATUS="$migration_234_status" RELEASE_DIR="$state_dir" bash "$source_dir/deploy/maintenance/release/migration-234-assert.sh" preflight >/dev/null
...
if [[ $profile == 233 || $profile == 234 ]]; then
  mark_stage migration_assertion_profile_233_upstream_account_key_unique
...
if [[ $profile == 234 ]]; then
  mark_stage migration_assertion_profile_234_upstream_health_observations
  ASSERT_CONTEXT_FILE="$migration_234_context" ASSERT_DB_CONTAINER=sub2api-postgres ASSERT_DB_USER="$database_owner" ASSERT_DB_NAME="$probe_db" MIGRATION_STATUS="$migration_234_status" RELEASE_DIR="$state_dir" bash "$source_dir/deploy/maintenance/release/migration-234-assert.sh" postflight >/dev/null
  migration_234_postflight_verified=true
fi
if [[ $profile == 195 || $profile == 197 || $profile == 198 || $profile == 199 || $profile == 202 || $profile == 206 || $profile == 207 || $profile == 208 || $profile == 209 || $profile == 210 || $profile == 212 || $profile == 213 || $profile == 215 || $profile == 232 || $profile == 233 || $profile == 234 ]]; then
...
jq -n --slurpfile manifest "$manifest" \
  ...
  --argjson migration_234_postflight_verified "$migration_234_postflight_verified" \
