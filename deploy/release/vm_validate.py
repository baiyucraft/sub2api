...
def main() -> None:
    ...
    remote_root = f"/opt/sub2api-deploy/release-input/{manifest['release_id']}-{secrets.token_hex(8)}"
    ...
    runner.run(
        "local_vm",
        f"test ! -e {remote_root} && install -d -m 700 {remote_root} && printf 'input_ready=true\\n'",
        {"input_ready"},
    )
    ...
    try:
        ...
        client = runner.connect("local_vm")
        try:
            sftp = client.open_sftp()
            try:
                for name in ("gate.json", "gate.sig", "candidate.tar.gz", "SHA256SUMS"):
                    sftp.get(f"{remote_output}/{name}", str(output / name))
            finally:
                sftp.close()
        finally:
            client.close()
