...
- 充值来源与邀请返利口径一致：成功完成的余额订单、成功完成的订阅订单和正数余额兑换码均计入；支付履约生成并立即兑换的余额码由订单计入，兑换码侧必须排除以防重复。
- 管理员直接调整余额仅在操作为正数 `add` 且现有“管理员充值参与邀请返利”开关当时开启时计入。`set`、`subtract` 不计入；该资格写入独立幂等事件，不能从正数 `admin_balance` 流水反推。
- 失败、取消、退款、并发码、订阅码和负数余额兑换码不计入充值门槛；同一邀请对象后续充值不重复计数。
...
- `activity_recharge_events`：保存无法从既有订单可靠重建的管理员正数 add 事件，以 `(source_type, source_key)` 唯一约束防止重复统计；历史 `admin_balance` 不回溯猜测 add/set。

抽奖事务先锁定可用机会，再生成奖励记录并通过现有余额服务入账；任一步失败整体回滚或进入可重试的明确 pending 状态。奖励金额使用整数分/Decimal，随机源使用后端安全随机源并记录规则版本与结算结果。

邀请达标事件在余额或订阅支付履约成功后触发快速同步，同时在读取 summary 时按统一充值来源幂等补齐；兑换码和管理员 add 因此也能参与累计。消费进度挂在最终成功计费结算之后，并复用现有 0 倍率语义。
