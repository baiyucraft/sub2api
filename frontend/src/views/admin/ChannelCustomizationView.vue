...
function normalizeRule(rule: Partial<ChannelCustomizationRule> = {}): ChannelCustomizationRule {
  ...
  return {
    ...
    request_message_match_mode: value.request_message_match_mode === 'regex' ? 'regex' : 'exact',
...
function summarizeConditions(rule: ChannelCustomizationRule) { const count = rule.methods.length + rule.exact_paths.length + rule.path_prefixes.length + rule.models.length + rule.user_agent_contains.length + Object.keys(rule.query_params || {}).length + (rule.request_message_text ? 1 : 0); return count ? t('admin.customization.conditionCount', { count }) : t('admin.customization.noConditions') }
function actionLabel(rule: ChannelCustomizationRule) { return t(rule.action === 'group_mapping' ? 'admin.customization.actionGroupMapping' : rule.action === 'model_mapping' ? 'admin.customization.actionModelMapping' : 'admin.customization.actionLocalResponse') }
...
async function saveSettings() {
  ...
  const invalid = payload.rules.some(rule => !rule.name || (!rule.api_key_ids.length && !rule.api_key_names.length && !rule.user_ids.length && !rule.user_emails.length) || (!rule.methods.length && !rule.exact_paths.length && !rule.path_prefixes.length && !rule.models.length && !rule.user_agent_contains.length && !Object.keys(rule.query_params).length && !rule.request_message_text) || (rule.target_model && (!rule.models.length || rule.target_model.length > 256)) || (rule.action === 'model_mapping' && !rule.target_model) || (rule.action === 'group_mapping' ? !rule.target_group_id || (!groupsLoadFailed.value && !availableGroups.value.some(group => group.id === rule.target_group_id)) : rule.action === 'local_response' && (rule.min_delay_ms < 0 || rule.min_delay_ms > 10000 || rule.max_delay_ms < 0 || rule.max_delay_ms > 10000 || rule.max_delay_ms < rule.min_delay_ms || rule.status_code < 100 || rule.status_code > 599 || ([204, 205, 304].includes(rule.status_code) && rule.response_body.trim() !== ''))))
