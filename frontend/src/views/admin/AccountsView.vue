    ...
    <TablePageLayout>
      ...
      <template #table>
        <AccountBulkActionsBar
          :selected-ids="selIds"
          :probe-eligible-count="selectedBillingProbeAccountIDs.length"
        ...
        >
          ...
          <template #cell-schedulable="{ row }">
            <div class="flex min-w-[132px] flex-col items-start gap-1">
              ...
              />
              <span
                v-if="row.upstream_scheduling_enabled === false"
                class="text-[11px] font-medium leading-4 text-amber-700 dark:text-amber-300"
              >
                {{ t('admin.accounts.upstreamSchedulingPaused') }}
              </span>
            </div>
          </template>
...
const menu = reactive<{show:boolean, acc:Account|null, anchorEl:HTMLElement|null}>({ show: false, acc: null, anchorEl: null })
const exportingData = ref(false)
const probingUpstreamBilling = reactive(new Set<number>())
const upstreamBillingEligibility = reactive(new Map<number, boolean>())
...
const isAccountBillingProbeEligible = (account: Account) =>
  account.type === 'apikey' &&
  account.upstream_config_id == null &&
...
const selectedBillingProbeAccountIDs = computed(() =>
  selIds.value.filter((id) => upstreamBillingEligibility.get(id) === true)
)
...
const load = async (options: AccountLoadOptions = {}) => {
  ...
  await baseLoad()
  const refreshTasks = [refreshAccountQualityBatch({ force: true })]
  if (options.refreshTodayStats !== false) {
    refreshTasks.push(refreshTodayStatsBatch({ force: true }))
  }
...
const syncAccountRefs = (nextAccount: Account) => {
  // List refreshes must not replace detail snapshots or reset unsaved editor state.
  // Detail mutations update the editor through its own response or an explicit refetch.
...
function accountHomepageUrl(row: Account): string {
  if (row.upstream_config_id != null && typeof row.upstream_site_url === 'string') {
    return sanitizeUrl(row.upstream_site_url)
  }
...
const openMenu = (a: Account, e: MouseEvent) => {
  menu.acc = a
  menu.anchorEl = e.currentTarget as HTMLElement | null
  menu.show = true
}
...
const handleBulkProbeUpstreamBilling = async () => {
	const accountIDs = [...selectedBillingProbeAccountIDs.value]
  if (accountIDs.length === 0) {
...
const handleManagedModelSyncRefresh = async () => {
  await load()
  const editingID = edAcc.value?.id
  if (editingID) {
    const detail = await loadAccountDetails({ id: editingID })
    if (detail && edAcc.value?.id === editingID) edAcc.value = detail
  }
}
