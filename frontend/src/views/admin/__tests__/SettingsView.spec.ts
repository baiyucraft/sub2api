...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("does not expose or submit retired STATE settings", async () => {
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
      openai_codex_ticket_enabled: true,
      openai_codex_ticket_harvest_proxy_url: "http://user:secret@pool.example.com:8080",
    });
    const wrapper = mountView();
    await flushPromises();
    ...
    expect(wrapper.html()).not.toContain("user:secret");
    await wrapper.find("form").trigger("submit.prevent");
    await flushPromises();
    expect(updateSettings).toHaveBeenCalled();
    const submitted = updateSettings.mock.calls[0]?.[0];
    expect(submitted).not.toHaveProperty("openai_codex_ticket_enabled");
    expect(submitted).not.toHaveProperty("openai_codex_ticket_harvest_proxy_url");
    wrapper.unmount();
  });
