...
vi.mock('@/api/admin', () => ({
  adminAPI: {
    accounts: {
      ...
      listWithEtag,
      getBatchTodayStats,
      getBatchQualityStats: vi.fn().mockResolvedValue({ data: { stats: {} } }),
      getBatchUsage: vi.fn().mockResolvedValue({}),
      getUpstreamBillingProbeSettings,
      delete: vi.fn(),
...
describe('admin AccountsView lite account list', () => {
  ...
  it('keeps detail snapshots and unsaved editor state across compact refreshes', async () => {
    vi.useFakeTimers()
    vi.spyOn(document, 'hidden', 'get').mockReturnValue(false)
    localStorage.setItem('account-auto-refresh', JSON.stringify({ enabled: true, interval_seconds: 5 }))
    const wrapper = mountView()
    ...
    await editButton!.trigger('click')
    await flushPromises()
    const detail = wrapper.findComponent(EditAccountModalStub).props('account')
    listWithEtag.mockResolvedValue({ notModified: false, etag: 'changed', data: {
      items: [{ ...listRow, current_concurrency: 1 }], total: 1, page: 1, page_size: 20, pages: 1
    } })
    // An already-running request may finish after the editor opens.
    await (wrapper.vm as any).$.setupState.refreshAccountsIncrementally()
    await flushPromises()
    expect(listWithEtag).toHaveBeenCalled()
    expect(wrapper.findComponent(EditAccountModalStub).props('account')).toBe(detail)
    expect(detail.extra).toEqual({ detail_only: true })
