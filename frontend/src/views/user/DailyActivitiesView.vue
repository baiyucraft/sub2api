        ...
        <div class="grid gap-4 md:grid-cols-2">
          <section class="card border-l-4 border-l-amber-400 p-5">
            <div class="flex items-start justify-between gap-4">
              <div><p class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('activities.dailyGift.title') }}</p><p class="mt-1 text-sm text-gray-500 dark:text-dark-400">{{ t('activities.dailyGift.description', { amount: formatCurrency(summary.daily_gift.threshold) }) }}</p></div>
            ...
            <button class="btn btn-primary mt-4 w-full" :disabled="!summary.daily_gift.eligible || summary.daily_gift.claimed || busy" @click="claimGift">{{ t('activities.dailyGift.button') }}</button>
          </section>

          <ActivityDrawCard type="recharge" :progress="summary.recharge" :title="t('activities.recharge.title')" :description="t('activities.recharge.description', { amount: formatCurrency(summary.recharge.threshold) })" :progress-label="t('activities.progress')" :available-draws-label="t('activities.availableDraws', { count: summary.recharge.available_draws })" :draw-one-label="t('activities.drawOne')" :draw-all-label="t('activities.drawAll')" :busy="busy" @draw="draw('recharge', $event)" />
          <ActivityDrawCard type="consumption" :progress="summary.consumption" :title="t('activities.consumption.title')" :description="t('activities.consumption.description', { amount: formatCurrency(summary.consumption.threshold) })" :progress-label="t('activities.progress')" :available-draws-label="t('activities.availableDraws', { count: summary.consumption.available_draws })" :draw-one-label="t('activities.drawOne')" :draw-all-label="t('activities.drawAll')" :busy="busy" @draw="draw('consumption', $event)" />

          <section class="card border-l-4 border-l-primary-500 p-5">
            <div class="flex items-start justify-between gap-4"><div><p class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('activities.invite.title') }}</p><p class="mt-1 text-sm text-gray-500 dark:text-dark-400">{{ t('activities.invite.description') }}</p></div><Icon name="users" size="lg" class="text-primary-500" /></div>
            ...
            <div class="mt-3 h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-dark-700"><div class="h-full rounded-full bg-primary-500 transition-all" :style="{ width: `${invitePercent}%` }" /></div>
