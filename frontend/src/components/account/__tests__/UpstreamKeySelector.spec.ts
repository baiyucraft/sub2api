...
describe('UpstreamKeySelector', () => {
  ...
  it('shows only rate_multiplier when legacy dedicated-rate metadata is present', () => {
    const wrapper = mount(UpstreamKeySelector, {
      props: {
        modelValue: 1,
        platform: 'openai',
    ...
    expect(wrapper.text()).toContain('admin.accounts.upstreamKeySelector.rate:{"rate":"0.06"}')
    expect(wrapper.text()).not.toContain('rateOverride')
    expect(wrapper.text()).not.toContain('rateDedicated')
  })
