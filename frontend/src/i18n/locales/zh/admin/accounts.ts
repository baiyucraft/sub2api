export default {
    accounts: {
      ...
      createAccount: '添加账号',
      upstreamKeySelector: {
        placeholder: '请选择上游 Key',
        searchPlaceholder: '搜索 Key 名称、分组、平台或倍率',
        empty: '没有可用的上游 Key',
        unnamedRemote: '上游 Key #{id}',
        unnamedLocal: '本地 Key #{id}',
        group: '分组: {group}',
        groupId: '分组 #{id}',
        ungrouped: '分组: 未分组',
        rate: '倍率: {rate}x',
      ...
      dataImportWarning: '导入将创建新账号与代理；可为单平台账号统一设置分组和优先状态，请确认已有数据不会冲突。',
      dataImportFile: '数据文件',
      ...
      dataImportProxyGroup: '代理组',
      dataImportProxyGroupHint: '仅当全部导入账号都是 OpenAI OAuth 或 Setup Token 时可选；选择后会覆盖 JSON 中的代理绑定。',
      dataImportSelectProxyGroup: '选择代理组（可选）',
      dataImportInvalidProxyGroup: '所选代理组不存在或已不可用，请重新选择。',
      dataImportOverrides: '统一覆盖设置',
      dataImportConcurrency: '并发数',
      dataImportRateMultiplier: '账号计费倍率',
      dataImportPriority: '账号优先级',
      dataImportCodexFingerprint: 'Codex 指纹收敛',
      dataImportGroups: '账号分组',
      dataImportGroupsHint: '所选分组和星标会应用到本次导入创建的全部账号。必须先选择分组，才能设为优先。',
      dataImportMixedPlatformGroupsDisabled: '当前文件包含多个账号平台，不能统一设置分组。请按平台分别导入后再设置。',
      dataImportPlatformUnavailable: '未能从导入数据识别账号平台，暂不能设置分组。',
      dataImportKeepOriginal: '留空以保留 JSON 原值',
      dataImportInvalidConcurrency: '并发数必须是非负整数。',
      dataImportInvalidRateMultiplier: '账号计费倍率必须是非负数字。',
      dataImportInvalidPriority: '账号优先级必须是大于等于 1 的整数。',
      ...
      quality: {
        ...
        cacheRateTitle: '缓存率 {rate}（缓存读 {numerator} / 提示词总量 {denominator}）',
        durationOnly: '首字样本不足，仅按总耗时评分，最高 69 分',
        ttftOnly: '仅按首字评分',
        cacheWeighted: '缓存率参与评分（首字 50% / 总耗时 15% / 缓存率 35%）',
        hint: '三行依次为：1H 活动状态 / 成功失败数 / 最近成功，1H 与 24H 等级分数 / 平均首 Token、总耗时与缓存率 / 样本数。缓存率为缓存读 Token ÷ 提示词总量（输入 + 缓存创建 + 缓存读取），并参与质量评分。跨模型历史观测仅供展示，不参与优先级或调度；TTFT Guard 仍按账号 + 模型独立运行。',
      ...
      upstreamImagePricing: {
        ...
        shared: '共享',
        independent: '独立',
        costTitle: '生图成本（{mode}倍率，{status}，价格来源：{source}）',
        costTooltipShared: '{tier} 成本 {cost}；共享倍率，使用账号倍率 {multiplier}×；状态：{status}；价格来源：{source}',
        costTooltipIndependent: '{tier} 成本 {cost}；独立生图倍率 {multiplier}×；状态：{status}；价格来源：{source}',
        badgeTitle: '生图状态：{status}；成本完整度：{complete}/3；价格来源：{source}；更新时间：{observedAt}',
        sourceUpstream: '上游返回',
        sourceBuiltinDefault: '内置默认价',
        sourceMixed: '上游 + 内置默认价',
        statusAvailable: '可用',
        statusPartial: '部分',
        statusStale: '过期',
      ...
      capacity: {
        ...
        proxyConcurrency: {
          available: '{name} 当前并发 {current} / {limit}，可继续调度',
          unavailable: '{name} 当前并发 {current} / {limit}，暂不可调度',
          unnamed: '代理 #{id}'
        },
        rpm: {
          full: '已达 RPM 上限',
      ...
      bulkEdit: {
        ...
        groupsAndPreferredHint: '将覆盖所有目标账号的分组及优先星标；未点星的已选分组会取消优先状态。',
      ...
      openai: {
        ...
        oauthAutoDisabled: {
          title: '官方响应映射自动禁用的模型',
          description: '这些 OpenAI OAuth 模型因上游返回了已配置的不同模型而被自动禁用。恢复后将重新允许调度。',
          restore: '恢复模型',
          empty: '当前没有自动禁用模型。',
          syncAvailable: '模型同步可用',
          syncPending: '模型同步中',
          syncError: '模型同步失败，仍保留禁用状态',
          syncErrorDetail: '同步错误：{error}'
        },
      },
      grok: {
      ...
      oauthModelMismatchRules: {
        title: 'OpenAI OAuth 模型映射禁用规则',
        description: '当 OAuth 账号请求模型 A、但上游返回模型 B 时，命中规则会自动禁用该账号的 A；当前请求不会被改写。',
        requestedModel: '请求模型（A）',
        responseModel: '上游响应模型（B）',
        remove: '删除',
        add: '添加规则',
        cancel: '取消',
        save: '保存',
      ...
      proxy: '代理',
      noProxy: '无代理',
      proxyGroupSummary: '{count} 个代理 · 每 IP 并发 {limit}',
      proxyBinding: {
        none: '不使用代理',
        single: '单代理',
        group: '代理组',
        selectGroup: '选择代理组',
        groupHint: '请求会在组内可用代理之间调度，并按每个代理 IP 独立统计并发。',
        loadFailed: '代理组加载失败，请刷新后重试。',
        groupOption: '{name} · {count} 个代理 · 每 IP {limit}'
      },
      concurrency: '并发数',
