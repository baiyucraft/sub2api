export default {
    ...
    channelMonitor: {
      ...
      form: {
        ...
        credentialMode: '凭据模式',
        credentialManual: '手工 Key',
        credentialManagedLocal: '托管本站 Key',
        credentialManagedHint: '使用本站 HTTPS 网关和指定分组，系统会自动创建并隐藏监控专用 Key。',
        provider: '平台',
        checkMode: '检查方式',
        checkModeProbe: '探活',
        ...
        apiModeZhipuNative: '智谱原生',
        apiModeZhipuNativeHint: '使用智谱原生 /api/paas/v4/chat/completions 探测 GLM 模型。',
        ...
        localGroup: '本站分组',
        localGroupPlaceholder: '请选择本站分组',
        localGroupLoading: '正在加载本站分组...',
    ...
    customization: {
      ...
      editRule: '编辑规则',
      noRules: '暂无定制规则',
      createFirst: '创建规则以定制指定探测请求。',
      loadError: '加载渠道定制失败',
      saveSuccess: '定制规则已保存',
      deleteSuccess: '定制规则已删除',
      observerSaved: '请求观察器设置已保存',
      deleteConfirm: '确定删除定制规则“{name}”吗？',
      nameRequired: '请输入规则名称',
      targetRequired: '至少添加一个 Key 或用户目标',
      invalidNumber: 'ID 必须是正整数',
      enabled: '已启用',
      disabled: '已停用',
      priority: '优先级',
      targets: '目标',
      ...
      statusCode: '状态码',
      responseBody: '响应体',
      responseBodyPlaceholder: '例如：status=ok',
      keyNames: 'API key 名称',
      keyNamesPlaceholder: 'maibon-gpt, maibon-grok',
      keyIds: 'API key ID',
      userIds: '用户 ID',
      userEmails: '用户邮箱',
      listPlaceholder: '使用逗号、分号或换行分隔',
      matchAny: '所有请求',
      matchOnly: '仅探测请求',
      matchExclude: '排除探测请求',
      observerEnabled: '启用请求观察器',
      observerHint: '观察器设置会立即生效，并复用现有管理员设置接口。',
      observerOutput: '输出文件',
      observerDisabled: '已停用',
      observerActive: '运行中',
      observerTargets: '观察目标',
      observerTargetHint: '名称、ID 和邮箱采用 OR 匹配。',
      rulesHint: '按页面顺序匹配，第一条命中的启用规则执行所选动作。目标和请求条件必须同时命中。',
      saveAll: '保存全部设置',
      unnamedRule: '未命名规则',
      conditions: '请求条件',
      response: '响应',
      ...
      targetGroupsLoadError: '渠道定制已加载，但目标分组列表暂时不可用。',
      unknownTargetGroup: '未知分组（{id}）',
      ruleName: '规则名称',
      moveUp: '上移',
      moveDown: '下移',
      status: '状态',
      observerPrivacy: '仅记录命中的观察目标。请求体会按现有审计规则脱敏，仅用于短期排查。',
      observerEnabledHint: '启用后记录匹配目标的请求和响应元数据。',
      noTargets: '未配置目标',
      conditionCount: '{count} 个条件组',
      noConditions: '未配置探针条件',
      methods: 'HTTP 方法',
      exactPaths: '精确路径',
      pathPrefixes: '路径前缀',
      ...
      modelsHint: '按请求体顶层 model 精确匹配，匹配时忽略大小写。',
      requestMessageMatchMode: '请求消息匹配模式',
      requestMessageMatchExact: '精确匹配',
      requestMessageMatchRegex: '正则匹配',
      requestMessageText: '请求消息文本',
      requestMessageTextPlaceholder: 'hi',
      requestMessageTextHint: '支持 Responses input、messages.content 和 Gemini contents.parts.text。仅匹配消息文本为该值的请求。',
      requestMessageRegexHint: '使用 Go RE2 正则匹配完整消息文本；请求中必须只能提取出一段用户文本。',
      minDelay: '最小延迟（毫秒）',
      maxDelay: '最大延迟（毫秒）',
      contentType: 'Content-Type',
      applyRule: '应用规则',
      validationError: '请检查规则名称、目标、请求条件、执行动作和对应配置。',
      saveError: '保存渠道定制失败',
