    ...
    settings: {
      ...
      features: {
        channelMonitor: {
          ...
          degradedThreshold: '总耗时降级阈值（秒）',
          degradedThresholdHint: 'V1 探测完整总耗时达到此值时，最终成功也标记为降级。范围 1 – 300 秒，默认 6 秒。',
          degradedRetryTolerance: '免降级重试次数',
          degradedRetryToleranceHint: '最终成功时允许的额外重试次数。范围 0 – 4 次，默认 2 次。',
          degradedSwitchTolerance: '免降级换号次数',
          degradedSwitchToleranceHint: '本轮探测允许的累计内部换号次数。范围 0 – 5 次，默认 3 次。',
          degradedPolicyRangeError: '渠道监控降级策略范围无效：阈值 1–300 秒，重试 0–4 次，换号 0–5 次。',
        ...
        dailyActivity: {
          title: '每日活动与抽奖',
          description: '所有资格、次数和奖励金额均由后端结算；活动奖励直接进入用户余额。',
          enabled: '启用每日活动',
          enabledHint: '关闭后用户端页面保持只读，领取和抽奖接口拒绝操作；历史奖励与机会保留。',
          dailyGift: '每日充值礼包',
          rechargeDraw: '充值阶梯抽奖',
          consumptionDraw: '消费阶梯抽奖',
          inviteDraw: '独立邀请抽奖',
          threshold: '门槛金额',
          inviteThreshold: '被邀请人充值门槛',
          inviteCount: '每次抽奖所需达标人数',
          rewardMin: '最小奖励',
          rewardMax: '最大奖励',
        },
      },
      ...
      gatewayRequestObserver: {
        title: '请求观察器',
        description: '仅按指定 API Key 或平台用户记录网关请求，便于短期排查探针请求格式和响应状态。默认关闭。',
        ...
        userIds: '平台用户 ID',
        userIdsHint: '填写平台用户 ID，每行或用逗号分隔。',
        userEmails: '平台用户邮箱',
        userEmailsPlaceholder: '例如：用户邮箱',
        userEmailsHint: '支持多个邮箱，每行或用逗号分隔。',
      ...
      scheduling: {
        ...
        capacityFailoverEnabled: '账号容量满载自动换号',
        capacityFailoverEnabledHint: '普通 OpenAI/OAuth 账号等待队列已满，或上游绑定账号返回 429 且同号重试耗尽时，在当前请求内排除该账号并尝试其他账号。上游 502、503 仍按原有故障转移策略处理。',
        capacityFailoverMaxSwitches: '最大换号次数',
        capacityFailoverMaxSwitchesHint: '范围 0–1000，默认 10。填 0 表示不限制；普通账号按共享并发池计数，上游账号按 429 导致的账号切换计数。',
        capacityFailoverExhaustedStatusCode: '容量耗尽 HTTP 状态码',
        capacityFailoverExhaustedStatusCodeHint: '范围 400–599，推荐 503。JSON error.code 始终为 gateway_capacity_exhausted。',
        capacityFailoverInvalid: '请填写 0–1000 的整数换号次数，以及 400–599 的整数 HTTP 状态码。',
        capacityFailoverSaved: '账号容量满载自动换号设置已保存',
        capacityFailoverSaveFailed: '保存账号容量满载自动换号设置失败',
      ...
      quotaNotify: {
        title: '管理员额度与渠道余额通知',
        description: '统一配置账号限额和上游渠道余额不足提醒',
        enabled: '启用账号限额通知',
        upstreamBalanceEnabled: '启用上游渠道余额不足提醒',
        upstreamBalanceHint: '默认开启；未配置有效通知邮箱时，自动发送到启用中的管理员注册邮箱',
        emails: '通知邮箱',
        emailsHint: '配置并验证后优先发送到这些邮箱；上游渠道余额提醒留空时会回退管理员注册邮箱',
        addEmail: '添加邮箱',
      ...
      smtp: {
        ...
        recipientRouting: '按收件域名切换 QQ SMTP',
        recipientRoutingHint: '命中域名后使用 QQ 发件地址，未命中或 QQ 连接失败时回退主 SMTP。',
        routingDomains: '路由域名',
        routingDomainsPlaceholder: 'qq.com\n*.example.com',
        routingDomainsHint: '每行一个精确域名或 *.example.com 通配符，统一按小写匹配。',
        qqProfile: 'QQ SMTP 配置',
        qqPasswordHint: '建议填写 QQ 邮箱授权码，不要使用登录密码。',
      },
      testEmail: {
        title: '发送测试邮件',
        ...
        profile: '发件配置',
        profileAuto: '自动按收件域名',
        profilePrimary: '主 SMTP',
        profileQQ: 'QQ SMTP',
        routePreview: '自动路由：{profile}',
      ...
      openaiTTFTGuard: {
        ...
        summary: 'TTFT 是请求到首个有效 Token 的耗时；仅影响账号调度，不修改优先级或计费。',
        tipsTitle: '判定规则',
        thresholdTip: '退化阈值 T：趋势均值达到 T 时判定退化；单次 ≥3T 或连续两次 ≥1.5T 会提前触发。',
        samplesTip: '最少样本数 N：趋势判定所需的有效样本数；范围 2-20，极慢请求可提前触发。',
        recoveryTip: '退化账号会暂时避开；连续 3 次 TTFT ≤0.6T 后恢复调度。',
        enabled: '启用 TTFT Guard',
        enabledHint: '样本达到门槛后，将近期 TTFT 持续偏高的 OpenAI 账号视为退化账号',
        degradationTTFTSeconds: '退化阈值（秒）',
        degradationTTFTSecondsHint: '趋势判定基准 T（5-300 秒）',
        minSamples: '最少样本数',
        minSamplesHint: '趋势判定所需的有效样本数（2-20）',
