export default {
    ...
    users: {
      ...
      customRate: '专属倍率',
      actualRate: '实际倍率',
      relativeRatePercent: '相对普通倍率 (%)',
      relativeRatePercentHint: '输入相对于分组普通倍率的百分比，例如普通倍率 0.5、输入 50% 即实际倍率 0.25。',
      relativeRatePercentUnavailable: '普通倍率为 0 时无法换算百分比',
      useDefaultRate: '使用默认',
      customRatePlaceholder: '留空使用默认',
      groupConfigUpdated: '分组配置更新成功',
      zeroRateConfirm: '确认将该用户在所选分组设置为 0 倍率？该用户不扣余额或订阅消费，但请求、Token、原始成本和统计仍保留；API Key 配额、限速和平台配额仍按分组原价计量，且该用户-分组组合不受利润控制。',
      replaceGroup: '替换分组',
    ...
    groups: {
      ...
      rateSaved: '专属倍率已保存',
      zeroRateConfirm: '确认保存 0 倍率专属设置？对应用户不扣余额或订阅消费，但请求、Token、原始成本和统计仍保留；API Key 配额、限速和平台配额仍按分组原价计量，且该用户-分组组合不受利润控制。',
      finalRate: '最终倍率',
