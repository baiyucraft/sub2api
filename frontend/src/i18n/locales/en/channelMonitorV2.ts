...
export default {
  channelMonitorV2: {
    ...
    admin: {
      ...
      v1CacheRateAliases: {
        title: 'V1 cache-rate group links',
        hint: 'Changes V1 channel-card display only; usage, billing, and V2 data remain separate.',
        description: 'Link two display groups so both cards show the larger valid cache rate from the pair.',
        add: 'Add mapping',
        uses: 'Use larger rate',
        empty: 'No group links configured',
        notEnoughGroups: 'At least two groups are required to configure a cache-rate link.',
      },
    },
  },
}
