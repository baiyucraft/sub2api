export default {
    accounts: {
      ...
      upstreamKeySelector: {
        ...
        rateDedicated: 'Dedicated rate: {dedicatedRate}x',
      ...
      dataImportWarning: 'Import will create new accounts and proxies. A single-platform import can apply groups and preferred status to every created account. Ensure existing data does not conflict.',
      ...
      dataImportProxyGroup: 'Proxy Group',
      dataImportProxyGroupHint: 'Available only when every imported account is OpenAI OAuth or Setup Token. The selected group overrides proxy bindings from the JSON.',
      dataImportSelectProxyGroup: 'Select proxy group (optional)',
      dataImportInvalidProxyGroup: 'The selected proxy group is missing or unavailable. Choose another group.',
      dataImportOverrides: 'Batch overrides',
      dataImportConcurrency: 'Concurrency',
      dataImportRateMultiplier: 'Account billing multiplier',
      dataImportPriority: 'Account priority',
      dataImportCodexFingerprint: 'Codex fingerprint convergence',
      dataImportGroups: 'Account groups',
      dataImportGroupsHint: 'Selected groups and preferred status apply to every account created by this import. Select a group before marking it preferred.',
      dataImportMixedPlatformGroupsDisabled: 'The selected files contain multiple account platforms, so groups cannot be applied as one batch. Import each platform separately to configure groups.',
      dataImportPlatformUnavailable: 'The account platform could not be identified from the import data, so groups cannot be configured.',
      dataImportKeepOriginal: 'Leave empty to keep the JSON value',
      dataImportInvalidConcurrency: 'Concurrency must be a non-negative integer.',
      dataImportInvalidRateMultiplier: 'Billing multiplier must be a non-negative number.',
      dataImportInvalidPriority: 'Account priority must be an integer greater than or equal to 1.',
      ...
      allQualityFilters: 'All Quality Filters',
      oauthType: 'OAuth',
      ...
      quality: {
        ...
        activity: {
          ...
          title: '{state}, {success} successful, {failed} failed, latest success {lastSuccess}',
          lastSuccessNow: 'now',
      ...
      upstreamImagePricing: {
        ...
        costTitle: 'Image cost ({mode} multiplier, {status}, source: {source})',
        costTooltipShared: '{tier} cost {cost}; shared multiplier using account rate {multiplier}x; status: {status}; source: {source}',
        costTooltipIndependent: '{tier} cost {cost}; independent image multiplier {multiplier}x; status: {status}; source: {source}',
        badgeTitle: 'Image status: {status}; cost completeness: {complete}/3; source: {source}; observed: {observedAt}',
        sourceUpstream: 'Upstream',
        sourceBuiltinDefault: 'Built-in default',
        sourceMixed: 'Upstream + built-in default',
      ...
      capacity: {
        ...
        proxyConcurrency: {
          available: '{name} concurrency {current} / {limit}; available for scheduling',
          unavailable: '{name} concurrency {current} / {limit}; currently unavailable',
          unnamed: 'Proxy #{id}'
        },
        rpm: {
      ...
      bulkEdit: {
        ...
        groupsAndPreferredHint: 'This replaces group membership and preferred stars for every target account. Selected groups without a star will no longer be preferred.',
      ...
      openai: {
        ...
        oauthAutoDisabled: {
          title: 'Models automatically disabled by official response mapping',
          description: 'These OpenAI OAuth models were disabled after the upstream returned a configured different model. Restore a model to allow it again.',
          restore: 'Restore',
          empty: 'No models are automatically disabled.',
          syncAvailable: 'Model sync available',
          syncPending: 'Model sync pending',
          syncError: 'Model sync failed; disabled state retained',
          syncErrorDetail: 'Sync error: {error}'
        },
      },
      grok: {
      ...
      oauthModelMismatchRules: {
        title: 'OpenAI OAuth model mismatch rules',
        description: 'When an OAuth account is requested with model A but the upstream reports model B, matching rules automatically disable A for that account. The request itself is not changed.',
        requestedModel: 'Requested model (A)',
        responseModel: 'Upstream response model (B)',
        remove: 'Remove',
        add: 'Add rule',
        cancel: 'Cancel',
        save: 'Save',
        saving: 'Saving…',
        loading: 'Loading…',
        required: 'Both model names are required',
        saved: 'Saved',
      ...
      proxy: 'Proxy',
      noProxy: 'No Proxy',
      proxyGroupSummary: '{count} proxies · {limit} concurrency per IP',
      proxyBinding: {
        none: 'No proxy',
        single: 'Single proxy',
        group: 'Proxy group',
        selectGroup: 'Select proxy group',
        groupHint: 'Requests use available members in the group, with concurrency tracked independently per proxy IP.',
        loadFailed: 'Failed to load proxy groups. Refresh and try again.',
        groupOption: '{name} · {count} proxies · {limit} per IP'
      },
