export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      sub2apiLogin: {
        authModeLabel: 'Sub2API 认证方式',
        authModeUserLogin: '邮箱密码',
        authModeManualJwt: '手动 JWT',
        emailLabel: 'Sub2API 登录邮箱 *',
        emailPlaceholder: '上游登录邮箱',
        passwordLabel: 'Sub2API 登录密码 *',
        passwordPlaceholder: '用于登录上游 Sub2API',
        passwordEditPlaceholder: '留空保留当前密码',
        jwtLabel: 'Sub2API JWT *',
        jwtPlaceholder: '粘贴你手动登录上游后获得的 JWT',
        jwtEditPlaceholder: '留空保留当前 JWT',
        refreshTokenLabel: 'Sub2API Refresh Token（可选）',
        refreshTokenPlaceholder: '粘贴登录响应中的 refresh_token，用于 JWT 过期后自动续期',
        refreshTokenEditPlaceholder: '留空保留当前 Refresh Token',
        hint: '该密码会作为高敏凭据保存，用于通过上游公开用户接口同步该 API Key 的分组倍率。',
        editHint: '留空会保留当前已保存密码。若要移除该密码，请先将上游类型切换为 newapi 或其他。',
        jwtHint: 'JWT 和 Refresh Token 会作为高敏凭据保存。填写 Refresh Token 后，JWT 过期时会自动续期；不填则过期后需要重新填写 JWT。',
        jwtEditHint: '留空会保留当前已保存 JWT 和 Refresh Token。JWT 过期且没有可用 Refresh Token 时，同步会提示失败。',
        emailRequired: '选择 sub2api 时必须填写上游登录邮箱',
        passwordRequired: '选择 sub2api 时必须填写上游登录密码',
        jwtRequired: '选择手动 JWT 时必须填写上游 JWT'
      },
