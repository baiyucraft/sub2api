export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      sub2apiLogin: {
        authModeLabel: 'Sub2API Auth Mode',
        authModeUserLogin: 'Email and password',
        authModeManualJwt: 'Manual JWT',
        emailLabel: 'Sub2API Login Email *',
        emailPlaceholder: 'Upstream login email',
        passwordLabel: 'Sub2API Login Password *',
        passwordPlaceholder: 'Used to log in to the upstream Sub2API',
        passwordEditPlaceholder: 'Leave empty to keep current password',
        jwtLabel: 'Sub2API JWT *',
        jwtPlaceholder: 'Paste the JWT you obtained after manually logging in upstream',
        jwtEditPlaceholder: 'Leave empty to keep current JWT',
        refreshTokenLabel: 'Sub2API Refresh Token (optional)',
        refreshTokenPlaceholder: 'Paste the refresh_token from the login response to renew expired JWTs automatically',
        refreshTokenEditPlaceholder: 'Leave empty to keep current Refresh Token',
        hint: 'This password is saved as a highly sensitive credential and used to sync this API key group multiplier through the upstream public user API.',
        editHint: 'Leave empty to keep the saved password. To remove it, switch the upstream type to newapi or Other first.',
        jwtHint: 'The JWT and Refresh Token are saved as highly sensitive credentials. With a Refresh Token, expired JWTs renew automatically; without one, enter a new JWT after expiry.',
        jwtEditHint: 'Leave empty to keep the saved JWT and Refresh Token. If the JWT expires without a usable Refresh Token, sync will show a failure.',
        emailRequired: 'Upstream login email is required when sub2api is selected',
        passwordRequired: 'Upstream login password is required when sub2api is selected',
        jwtRequired: 'Upstream JWT is required when manual JWT is selected'
      },
